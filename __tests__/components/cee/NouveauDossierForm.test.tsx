...
const BASE_PROPS = {
  ...
  rgeQualifications: [
    { code: 'QualiPAC', date_fin: '2027-12-31' },
  ],
  isCertified: true,
}
...
describe('NouveauDossierForm -- etape Client', () => {
  ...
  it('ne passe pas etape 2 si email invalide', () => {
    ...
    fireEvent.change(screen.getByLabelText(/Adresse du chantier/i), { target: { value: '12 rue Test' } })
...
describe('NouveauDossierForm -- navigation etape 1 vers 2', () => {
  function fillStep1AndNext() {
    ...
    fireEvent.change(screen.getByLabelText(/T\u00e9l\u00e9phone/i), { target: { value: '0612345678' } })
    fireEvent.change(screen.getByLabelText(/Adresse du chantier/i), { target: { value: '12 rue de la Paix' } })
    fireEvent.change(screen.getByLabelText(/Code postal/i), { target: { value: '75011' } })
    fireEvent.click(screen.getByRole('button', { name: /Suivant/i }))
  }
...
describe('NouveauDossierForm -- submit OK', () => {
  ...
  it('appelle POST /api/cee/dossiers avec les bonnes donnees', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Ann\u00e9e de construction/i), { target: { value: '1985' } })
    fireEvent.click(screen.getByRole('button', { name: /Suivant/i }))

    // Etape 3
    await waitFor(() => expect(screen.getByLabelText(/Op\u00e9ration CEE/i)).toBeInTheDocument())
    fireEvent.change(screen.getByLabelText(/Op\u00e9ration CEE/i), { target: { value: 'BAR-TH-171' } })
    ...
    // Etape 4 apercu
    await waitFor(() => expect(screen.getByRole('button', { name: /Cr\u00e9er le dossier/i })).toBeInTheDocument())
