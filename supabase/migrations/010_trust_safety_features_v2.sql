-- ===========================================
-- Trust & Safety Features Migration v2
...
-- Simple policy: users can view their own bookings as client
...
-- KYC Policies
DROP POLICY IF EXISTS "Users can view own KYC profile" ON kyc_profiles;
CREATE POLICY "Users can view own KYC profile" ON kyc_profiles
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own KYC profile" ON kyc_profiles;
...
DROP POLICY IF EXISTS "Participants can add dispute messages" ON dispute_messages;
...
DROP POLICY IF EXISTS "Clients can create escrows" ON escrow_transactions;
CREATE POLICY "Clients can create escrows" ON escrow_transactions
  FOR INSERT WITH CHECK (auth.uid() = client_id);

DROP POLICY IF EXISTS "Participants can update escrows" ON escrow_transactions;
...
DROP POLICY IF EXISTS "Users can view escrow milestones" ON escrow_milestones;
...
-- 2FA Policies
DROP POLICY IF EXISTS "Users can view own 2FA" ON two_factor_secrets;
CREATE POLICY "Users can view own 2FA" ON two_factor_secrets
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own 2FA" ON two_factor_secrets;
...
DROP POLICY IF EXISTS "Users can manage own backup codes" ON two_factor_backup_codes;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_kyc_profiles_updated_at') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_two_factor_secrets_updated_at') THEN
