-- =============================================================================
-- Migration 363: Fix dispatch_lead() location filtering
-- ServicesArtisans — 2026-03-28
-- =============================================================================
-- BUG: dispatch_lead() receives p_city and p_postal_code but NEVER uses them
-- in the WHERE clause when coordinates are unavailable (v_lead_geo IS NULL).
-- Result: a client in Lille gets matched with artisans from Paris, Lyon, etc.
--
-- FIX:
--   1. When coordinates exist → use ST_DWithin radius (existing behavior)
--   2. When no coordinates but city is known → match by city name (unaccent)
--   3. When no coordinates and no city but postal code → match by department
--   4. Artisans with NO location data are excluded from located requests
--   5. Change require_same_department default to TRUE (safer default)
-- =============================================================================

-- Ensure unaccent extension is available for accent-insensitive matching
CREATE EXTENSION IF NOT EXISTS unaccent;

-- Update require_same_department default to TRUE (safer default)
ALTER TABLE public.algorithm_config
  ALTER COLUMN require_same_department SET DEFAULT true;

-- Also update the existing row(s) to use the safer default
UPDATE public.algorithm_config
SET require_same_department = true
WHERE require_same_department = false;

-- Recreate dispatch_lead with proper location filtering
...
BEGIN
  ...
  -- Do we have ANY location info for this lead?
  ...
  FOR v_provider IN
    ...
    WHERE p.is_active = true
      ...
      -- =====================================================================
      -- LOCATION FILTERING (fixed in migration 363)
      -- Priority: coordinates > city name > department > no filter
      -- =====================================================================
      AND (
        -- Case 1: No location info on lead at all → no location filter (nationwide)
        NOT v_has_location_info
        OR
        -- Case 2: Lead has coordinates AND provider has coordinates → radius match
        (v_lead_geo IS NOT NULL AND p.location IS NOT NULL
         AND ST_DWithin(p.location, v_lead_geo, v_config.geo_radius_km * 1000))
        OR
        -- Case 3: No coordinates but city is known → city name match (accent-insensitive)
        ...
        OR
        -- Case 4: No coordinates, no city match, but postal code → department match
      ...
      -- Department double-check (when config requires it AND we have department info)
      ...
      -- Cooldown between lead assignments
...
COMMENT ON FUNCTION dispatch_lead IS
  'Distribue un lead aux artisans selon algorithm_config. Filtre par localisation: coordonnées > ville > département. Migration 363.';

-- Index to support city-name matching in dispatch (accent-insensitive)
CREATE INDEX IF NOT EXISTS idx_providers_city_unaccent
  ON providers (lower(address_city));

-- Index to support department matching via postal code prefix
CREATE INDEX IF NOT EXISTS idx_providers_postal_prefix
  ON providers (left(address_postal_code, 2));
