...
import { Droplets, Zap, HardHat, PaintBucket, Hammer, ShieldCheck, Star, MapPin, Shield, Key, Flame, TreeDeciduous, ArrowRight, Clock, CheckCircle, ClipboardList } from 'lucide-react'
...
function renderStars(rating: number) {
  return [1, 2, 3, 4, 5].map(i => (
    <span key={i} className={i <= Math.round(rating) ? 'text-amber-400' : 'text-charcoal-200'}>&#9733;</span>
  ))
}
...
function ClayFAQSection() {
  ...
  return (
    <div className="max-w-[1320px] mx-auto px-6 md:px-10 py-24">
      ...
      <h2 className="font-heading font-black tracking-[-0.04em] leading-tight text-charcoal-900 text-center mb-12" style={{ fontSize: 'clamp(2rem,3.5vw,2.8rem)' }}>
        Tout ce que vous devez savoir.
      </h2>
      <div className="max-w-3xl mx-auto space-y-3">
        {faqs.map((faq) => (
          <details key={faq.q} className="group rounded-2xl bg-sand-50 border border-sand-300/60 transition-shadow duration-300 hover:shadow-soft">
            <summary className="flex items-center justify-between cursor-pointer px-6 py-4 text-left text-base font-bold text-charcoal-900 list-none [&::-webkit-details-marker]:hidden">
              <span>{faq.q}</span>
              <svg className="w-5 h-5 text-primary-400 shrink-0 ml-4 transition-transform duration-300 group-open:rotate-180" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
    ...
    <>
      ...
      <section className="relative bg-gradient-sand overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full opacity-[0.04]" style={{ background: 'radial-gradient(circle, #E86B4B 0%, transparent 70%)', filter: 'blur(100px)' }} />
          <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full opacity-[0.03]" style={{ background: 'radial-gradient(circle, #3D8B68 0%, transparent 70%)', filter: 'blur(80px)' }} />
        </div>
      ...
      <ScrollReveal as="section">
        <div className="bg-white py-16 md:py-20">
          <div className="max-w-[1320px] mx-auto px-5 md:px-10">
            ...
            <Link href="/services" className="text-sm font-bold text-primary-400 hover:text-primary-600 transition-colors block text-center mt-6 sm:hidden">
                  ...
                  <ScrollReveal key={step.n} delay={i * 0.12}>
                    <div className="text-center relative z-10">
                      ...
                      <div className="relative mx-auto mb-5 w-24 h-24 flex items-center justify-center">
                        <span className="absolute inset-0 flex items-center justify-center font-heading text-7xl font-black text-primary-100 select-none" aria-hidden="true">
                          {step.n}
                        </span>
                      ...
                      <div className="font-heading text-lg font-bold text-charcoal-900 mb-2">{step.title}</div>
                      <p className="text-sm text-charcoal-500 leading-relaxed max-w-xs mx-auto">{step.desc}</p>
                    </div>
                  </ScrollReveal>
                )
              })}
        ...
        <div className="bg-white py-16 md:py-20">
          <div className="max-w-[1320px] mx-auto px-5 md:px-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10 mb-14">
              {[
                { value: countStr, label: 'artisans vérifiés', icon: ShieldCheck, color: 'text-accent-600' },
                { value: '98%', label: 'satisfaction', icon: Star, color: 'text-amber-500' },
                { value: '23 min', label: 'temps de réponse moyen', icon: Clock, color: 'text-primary-400' },
              ].map(stat => {
                ...
                return (
                  <div key={stat.label} className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-heading text-2xl font-black text-charcoal-900">{stat.value}</div>
                    ...
                    <div className="rounded-3xl overflow-hidden transition-all duration-300 bg-white border border-sand-200 hover:shadow-card-hover hover:-translate-y-1">
                      ...
                      <div className="px-5 pb-5 pt-4">
                        ...
                        <div className="flex items-center gap-1.5 mb-3">
                          ...
                          <span className="text-sm font-bold text-charcoal-900">{ratingDisplay}</span>
                          <span className="text-xs text-charcoal-400">({a.review_count ?? 0} avis)</span>
                        </div>
