...
export default function Step2Revenus({ state, dispatch }: Props) {
  ...
  return (
    <form onSubmit={onNext} className="space-y-5" noValidate>
      <h3 className="text-lg font-semibold text-slate-900">Étape 2 — Vos revenus</h3>
      <p className="text-sm text-slate-600">
        Zone détectée : <strong>{idf ? 'Île-de-France' : 'Hors Île-de-France'}</strong>
      </p>

      <div>
        <label htmlFor="rfr" className="mb-1 block text-sm font-medium text-slate-800">
        ...
        {showTooltip ? (
          <p id="rfr-help" className="mt-2 rounded-md bg-slate-50 p-3 text-xs text-slate-700">
