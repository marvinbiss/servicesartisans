...
const CLASSES: Record<CeeDossierStatus, string> = {
  ...
  depose_delegataire: 'bg-blue-50 text-blue-700 border-blue-200',
  depose_pncee: 'bg-blue-100 text-blue-800 border-blue-200',
...
export default function StatusBadge({ status, className = '' }: StatusBadgeProps) {
  ...
  return (
    <span
      className={`${base} ${CLASSES[status] ?? 'bg-gray-100 text-gray-700 border-gray-200'} ${className}`.trim()}
      role="status"
