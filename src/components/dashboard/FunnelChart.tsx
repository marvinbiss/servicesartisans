...
const stageColors: Record<string, string> = {
  created: 'bg-primary-400',
  dispatched: 'bg-primary-500',
  viewed: 'bg-yellow-500',
  quoted: 'bg-green-500',
  accepted: 'bg-accent-500',
  completed: 'bg-green-700',
  declined: 'bg-charcoal-400',
...
export function FunnelChart({ steps, title }: FunnelChartProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-sand-300 p-6">
      {title && <h3 className="text-sm font-semibold text-charcoal-900 mb-4">{title}</h3>}
      <div className="space-y-3">
        {steps.map((step) => {
          const widthPct = Math.max((step.count / maxCount) * 100, 2)
          const color = stageColors[step.stage] || 'bg-charcoal-400'
            ...
            <div key={step.stage}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-charcoal-400 tabular-nums w-12 text-right">
                    {step.rate.toFixed(1)}%
                  </span>
                </div>
              </div>
