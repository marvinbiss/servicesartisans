 ...
 */
export default function MapViewController({ selectedProvider, providers }: MapViewControllerProps) {
  const map = useMap()
  ...
  useEffect(() => {
    ...
    if (
      ...
      map.flyTo([provider.latitude, provider.longitude], Math.max(map.getZoom(), 15), {
        duration: 1.5,
        easeLinearity: 0.25,
      })
    }
