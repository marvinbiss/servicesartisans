  ...
  const navigableCityItems = useMemo(() => {
    ...
    if (!hasTypedCity && recentSearches.length > 0) {
      ...
      return recentSearches.map((name) => {
        const match = (_allVilles || villesLight).find(
          (v) => normalizeText(v.name) === normalizeText(name)
        )
        return (
          match ||
          ({
                    ...
                    <div className="p-2" ref={serviceListRef}>
                      ...
                      {filteredServices.map((service, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-10 h-10 rounded-lg bg-gradient-to-br ${service.color} flex items-center justify-center shadow-sm transition-transform duration-150 ${
