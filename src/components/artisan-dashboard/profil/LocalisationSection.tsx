...
export function LocalisationSection({ provider, onSaved }: LocalisationSectionProps) {
  ...
  const radiusValue =
    typeof formData.intervention_radius_km === 'number' ? formData.intervention_radius_km : 30
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            <label
              htmlFor="localisation-postal-code"
              className="block text-sm font-medium text-charcoal-700 mb-2"
            >
              Code postal
            </label>
            <input
              ...
              onChange={(e) =>
                setField(
                  'address_postal_code',
                  e.target.value.replace(/\D/g, '').slice(0, 5) || null
                )
              }
              maxLength={5}
              placeholder="75001"
              className="w-full px-4 py-3 border border-sand-400 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-primary-400"
            />
            <p className="text-xs text-charcoal-500 mt-1">5 chiffres</p>
          </div>
        </div>
