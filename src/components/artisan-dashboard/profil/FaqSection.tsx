        ...
        <div className="space-y-4">
          {faqItems.map((item, index) => {
            const questionTooShort =
              item.question.trim().length > 0 && item.question.trim().length < MIN_QUESTION_LENGTH
            const answerTooShort =
              item.answer.trim().length > 0 && item.answer.trim().length < MIN_ANSWER_LENGTH
            return (
              <div key={index} className="border border-sand-300 rounded-lg p-4 relative">
                <button
                  type="button"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-primary-400 text-sm ${
                        questionTooShort ? 'border-amber-300' : 'border-sand-400'
                      }`}
                    />
                    {questionTooShort && (
                      <p className="text-xs text-amber-600 mt-0.5">
                        {MIN_QUESTION_LENGTH} caractères minimum
                      </p>
                    )}
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-primary-400 text-sm resize-y ${
                        answerTooShort ? 'border-amber-300' : 'border-sand-400'
                      }`}
                    />
                    {answerTooShort && (
                      <p className="text-xs text-amber-600 mt-0.5">
                        {MIN_ANSWER_LENGTH} caractères minimum
                      </p>
                    )}
                  </div>
                </div>
