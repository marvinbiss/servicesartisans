...
import { Star, ThumbsUp, MessageCircle, Loader2, X, ArrowUpDown } from 'lucide-react'
...
export function AvisTab() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-charcoal-900">Avis reçus</h2>
        <p className="text-charcoal-600 text-sm">Consultez et répondez aux avis de vos clients</p>
      </div>
            ...
            <div className="flex justify-center mb-2">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`w-6 h-6 ${
                    i < Math.round(stats.moyenne) ? 'text-yellow-400 fill-current' : 'text-sand-500'
                  }`}
                />
              ))}
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-charcoal-900">Derniers avis</h3>
        ...
        <div className="space-y-6">
          {avis.map((item) => (
            <div key={item.id} className="border-b border-sand-200 pb-6 last:border-0 last:pb-0">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-green-100 text-sm">
              ...
              professionnalisme et encourage d'autres clients à laisser leur avis. Les artisans qui
              répondent aux avis peuvent recevoir davantage de demandes.
            </p>
          </div>
        </div>
      </div>
