...
interface ProfileSubscriptionStats {
  totalActive: number
  premiumCount: number
  basicCount: number
  gratuitCount: number
...
interface ProfilesSubscriptionResponse {
  profiles: ProfileSubscription[]
  stats: ProfileSubscriptionStats | null
  totalPages: number
  total: number
}
...
export default function AdminSubscriptionsPage() {
  ...
  const { data, isLoading, error, mutate } = useAdminFetch<ProfilesSubscriptionResponse>(
  ...
  const emptyStats: ProfileSubscriptionStats = {
    totalActive: 0,
    premiumCount: 0,
    basicCount: 0,
    gratuitCount: 0,
  ...
  const getPlanBadge = (plan: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors[plan] || colors.gratuit}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-2xl font-bold text-gray-900">{displayStats.totalActive}</p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-2xl font-bold text-blue-600">{displayStats.premiumCount}</p>
            <p className="text-sm text-gray-500 mt-1">Comptes Premium</p>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-2xl font-bold text-blue-600">{displayStats.basicCount}</p>
            <p className="text-sm text-gray-500 mt-1">Comptes Basique</p>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-2xl font-bold text-gray-900">{displayStats.gratuitCount}</p>
            <p className="text-sm text-gray-500 mt-1">Comptes Gratuits</p>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[700px]" aria-label="Liste des abonnements">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {profiles.length === 0 ? (
                      ...
                      <tr key={profile.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">{getPlanBadge(profile.subscription_plan)}</td>
                        <td className="px-6 py-4">{getStatusBadge(profile.subscription_status)}</td>
                        <td className="px-6 py-4">
                          {profile.stripe_customer_id ? (
                            <span className="text-xs font-mono text-gray-500">{profile.stripe_customer_id}</span>
                          ) : (
