'use client'

import { CreditCard } from 'lucide-react'

export default function AdminSubscriptionsPage() {
  return (
    <div className="min-h-screen bg-sand-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-charcoal-900">Gestion des Abonnements</h1>
          <p className="text-charcoal-500 mt-1">Suivi des plans d'abonnement des utilisateurs</p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-sand-100 p-12 text-center">
          <div className="flex justify-center mb-4">
            <div className="p-4 bg-sand-100 rounded-full">
              <CreditCard className="w-10 h-10 text-charcoal-400" />
            </div>
          </div>
          <h2 className="text-lg font-semibold text-charcoal-700 mb-2">
            Fonctionnalité abonnements non disponible
          </h2>
          <p className="text-sm text-charcoal-500 max-w-md mx-auto">
            La gestion des abonnements n'est pas encore configurée pour cette plateforme. Revenez
            plus tard ou contactez l'équipe technique.
          </p>
        </div>
      </div>
    </div>
  )
}
