...
export async function GET() {
  try {
    ...
    const { data, error } = await getAiSettings(supabase)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: existing } = await getAiSettingsId(supabase)

    if (!existing) {
      return NextResponse.json(
        { success: false, error: { message: 'Settings non trouvés' } },
        { status: 404 }
      )
    }

    const { data, error } = await updateAiSettings(
      supabase,
      existing.id,
      parsed.data,
      authResult.admin.id
