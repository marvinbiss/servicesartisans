...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await getCampaignById(supabase, id)

    if (error || !data) {
      return NextResponse.json(
        { success: false, error: { message: 'Campagne non trouvée' } },
        { status: 404 }
      )
    }
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const supabase = createAdminClient()
    const { data, error } = await updateCampaign(supabase, id, sanitizedData)
  ...
  try {
    ...
    if (campaign?.status !== 'draft') {
      return NextResponse.json(
        {
          success: false,
          error: { message: 'Seules les campagnes en brouillon peuvent être supprimées' },
        },
        { status: 400 }
      )
    }

    const { error } = await deleteCampaign(supabase, id)
