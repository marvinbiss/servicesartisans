...
import { generateWithFallback, shouldEscalate } from '@/lib/prospection/ai-response'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!contact) {
      logger.warn('Incoming message from unknown number', { from })
      return new NextResponse('OK', { status: 200 })
    }
    ...
    if (aiSettings?.auto_reply_enabled) {
      ...
      if ((conversation.ai_replies_count || 0) < (aiSettings.max_auto_replies || 3)) {
        try {
          ...
          if (channel === 'whatsapp') {
            await sendWhatsAppReply(from, aiResult.content)
          } else {
            await sendProspectionSMS({ to: from, body: aiResult.content })
          }
