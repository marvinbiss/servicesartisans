...
const listQuerySchema = z.object({
  ...
  sortBy: z
    .enum(['updated_at', 'created_at', 'title', 'status', 'page_type', 'published_at'])
    .optional()
    .default('updated_at'),
...
export async function GET(request: Request) {
  try {
    ...
    const { page, pageSize, page_type, status, search, sortBy, sortOrder, show_inactive } =
      parsed.data
    ...
    let query = supabase
      .from('cms_pages')
      .select(
        'id, slug, page_type, title, status, meta_title, meta_description, published_at, updated_at, created_at, author, category, tags, sort_order, is_active, excerpt, featured_image, service_slug, location_slug',
        { count: 'exact' }
      )
    ...
    if (error) {
      if (error.code === '23505') {
        return NextResponse.json(
          {
            success: false,
            error: { message: 'Un slug identique existe déjà pour ce type de page' },
          },
          { status: 409 }
        )
      }
