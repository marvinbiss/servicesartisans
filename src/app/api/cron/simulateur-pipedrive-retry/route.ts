...
const MAX_FAILURES_PER_RUN = BATCH_SIZE
const LEASE_NAME = 'cron_simulateur_pipedrive_retry'
...
export const GET = withCronCheckIn('cron-simulateur-pipedrive-retry', async (request: Request) => {
  ...
  if (leaseErr) {
    logger.error('simulateur-pipedrive-retry: lease error', { error: leaseErr.message })
    return NextResponse.json({ error: 'lease_error' }, { status: 500 })
  }
  if (acquired !== true) {
    logger.info('simulateur-pipedrive-retry: skipped (lease already held)')
