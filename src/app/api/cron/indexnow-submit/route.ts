...
const LEASE_NAME = 'cron_indexnow_submit'
...
export const GET = withCronCheckIn('cron-indexnow-submit', async (request: Request) => {
  return await Sentry.withMonitor(
    ...
    async () => {
      ...
      if (leaseErr) {
        logger.error('IndexNow cron: lease error', undefined, {
      ...
      if (acquired !== true) {
        logger.info('IndexNow cron: skipped (lease already held)', { lease: LEASE_NAME })
      ...
      try {
        ...
        // SLA-99.9 : wall-clock guard — si la construction de la liste a déjà
        // mangé le budget (DB lente), on skip le POST IndexNow plutôt que de
        // risquer un kill Vercel avant release du lease.
        if (Date.now() - startedAt > MAX_RUNTIME_MS) {
          logger.warn('IndexNow cron: wall-clock budget exhausted before submit, skipping', {
            action: 'indexnow-cron',
