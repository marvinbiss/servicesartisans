...
export const GET = withCronCheckIn('cron-voice-stats', async (request: NextRequest) => {
  ...
  try {
    ...
    const { error } = await supabase.from('voice_stats_daily').upsert(stats, { onConflict: 'date' })
    ...
    logger.info('voice-stats completed', {
      date: dateStr,
      total: stats.total_calls,
      qualified: stats.qualified_a + stats.qualified_b,
    })

    return NextResponse.json({ success: true, data: stats })
  } catch (error) {
    logger.error('voice-stats: unexpected error', { error })
