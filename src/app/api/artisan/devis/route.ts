...
export async function POST(request: Request) {
  try {
    ...
    const { devis_request_id, amount, description, validity_days = 30, items } = body

    if (!devis_request_id || !amount) {
      return NextResponse.json(
        { error: 'ID de demande et montant requis' },
        { status: 400 }
      )
    }
...
export async function PUT(request: Request) {
  try {
    ...
    const body = await request.json()
    const { devis_id, status } = body

    if (!devis_id || !status) {
      return NextResponse.json(
        { error: 'ID du devis et statut requis' },
        { status: 400 }
      )
    }
