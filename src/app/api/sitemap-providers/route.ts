...
const specialtyToSlug: Record<string, string> = {
  ...
  // Pivot pure-play BTP énergétique 2026-05-02 : spécialités ADEME des
  // 5 métiers hors thèse retirées (jardinier, paysagiste, nettoyage,
  // alarme-securite, demenageur, + alias amenagement-exterieur, alarme,
  // securite, videosurveillance, demenagement, nettoyage-professionnel).
  // Pivot RGE 2026-05-01 : 16 spécialités ADEME des métiers Tier C niche
  // (solier, moquettiste, terrassier/terrassement, métallier/métallerie,
  //  ferronnier/ferronnerie, poseur-de-parquet/parqueteur, miroitier, storiste/
  //  store/volet, architecte-interieur, decorateur/decoration, domoticien/
  //  domotique, pisciniste/piscine, antenniste/antenne, ascensoriste/ascenseur,
  //  geometre/geometre-expert, desinsectisation/desinsectiseur/nuisibles,
  //  deratisation/deratiseur) retirées du mapping → providers concernés
  //  filtrés du sitemap (serviceSlug undefined → entry skipped).
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Latest provider updated_at = driver Last-Modified pour ce shard.
    const latestUpdate = allProviders.reduce<string | undefined>(
      (acc, p) => (p.updated_at && (acc === undefined || p.updated_at > acc) ? p.updated_at : acc),
      undefined
    )
