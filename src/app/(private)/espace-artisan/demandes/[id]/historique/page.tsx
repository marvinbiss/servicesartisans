  ...
  return (
    <div className="min-h-screen bg-sand-50">
      <div className="bg-white border-b">
        <div className="max-w-3xl mx-auto px-4 py-3 text-sm text-charcoal-500">
          ...
          <span className="mx-2">/</span>
          <Link href={`/espace-artisan/demandes/${id}`} className="hover:text-charcoal-900">
            Détail
          </Link>
          <span className="mx-2">/</span>
          <span className="text-charcoal-900">Historique</span>
        </div>
      </div>

      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link
          href={`/espace-artisan/demandes/${id}`}
          className="inline-flex items-center gap-1 text-sm text-charcoal-500 hover:text-charcoal-900 mb-6"
        >
          <ArrowLeft className="w-4 h-4" />
          Retour à la demande
        </Link>

        <h1 className="text-xl font-bold text-charcoal-900 mb-6">Historique du lead</h1>
        ...
        {events.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            <Clock className="w-12 h-12 text-sand-500 mx-auto mb-4" />
            <p className="text-charcoal-500">Aucun événement enregistré</p>
          </div>
        ) : (
              ...
              {events.map((event) => {
                ...
                return (
                  <div key={event.id} className="relative flex gap-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${config.color}`}
                    >
                      <Icon className="w-4 h-4" />
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-sand-200 p-4 flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-charcoal-900">{config.label}</span>
