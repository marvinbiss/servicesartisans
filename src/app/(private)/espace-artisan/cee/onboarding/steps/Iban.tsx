...
export default function Iban({ partner, onNext, onPartnerRefresh }: IbanProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 429) {
        setApiError('Trop de tentatives. Veuillez patienter une minute avant de réessayer.')
        return
      }
      ...
      if (!res.ok) {
        setApiError(json?.error?.message || 'Une erreur est survenue. Veuillez réessayer.')
        return
      }
  ...
  if (isAlreadySaved && !success && !showModifyForm) {
    return (
      <section className="p-6 sm:p-8" aria-labelledby="iban-heading" data-testid="step-iban">
        <header className="mb-6">
          ...
          <h2 id="iban-heading" className="font-heading text-2xl font-bold text-charcoal-900">
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-accent-600">
              Compte se terminant par <strong>···· {partner?.iban_last4}</strong>
            </p>
          </div>
        </div>
    ...
    <section className="p-6 sm:p-8" aria-labelledby="iban-heading" data-testid="step-iban">
      <header className="mb-6">
        ...
        <p className="mt-2 text-charcoal-500">
          Renseignez votre IBAN pour recevoir vos primes CEE. Ces données sont chiffrées (AES-256)
          et seuls les 4 derniers chiffres sont visibles.
        </p>
      </header>
      ...
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-sand-200 bg-sand-50 p-4">
        ...
        <p className="text-sm text-charcoal-500">
          Votre IBAN est stocké de manière sécurisée conformément à la directive PSD2 et aux
          exigences du PNCEE (arrêté 2/11/2023).
        </p>
      </div>
