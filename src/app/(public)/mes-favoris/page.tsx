      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {!loading && count === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-charcoal-900 max-w-md mx-auto mb-8">
              Parcourez nos artisans et cliquez sur{' '}
              <Heart className="inline w-4 h-4 text-red-400" /> pour les sauvegarder ici.
            </p>
            <Link
              href="/recherche"
              className="inline-flex items-center gap-2 px-6 py-3 bg-primary-500 hover:bg-primary-600 text-white font-semibold rounded-xl shadow-md shadow-primary-400/20 transition-all duration-200"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {providers.map((provider) => {
              ...
              return (
                <div
                  key={provider.id}
                  className="relative group bg-white rounded-2xl border border-charcoal-200 overflow-hidden hover:shadow-xl transition-all duration-300"
                >
                  {/* Favorite button */}
                  <div className="absolute top-3 right-3 z-10">
                    <FavoriteButton
                      providerId={provider.stable_id}
                  ...
                  <Link href={href} className="block p-5">
                    ...
                    <div className="flex items-start gap-4 mb-3">
                      <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary-400 to-primary-500 flex items-center justify-center text-white text-xl font-bold shadow-sm flex-shrink-0">
                        {provider.name.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <h3 className="font-semibold text-lg text-charcoal-900 group-hover:text-primary-500 transition-colors truncate">
                            {provider.name}
                          </h3>
                        ...
                        {provider.specialty && (
                          <p className="text-sm text-charcoal-900 mt-0.5">{provider.specialty}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {provider.address_city && (
                      <div className="flex items-center gap-1.5 text-sm text-charcoal-900 mb-3">
                        <MapPin className="w-4 h-4 flex-shrink-0" />
                        <span>
                          {provider.address_city}
                          {provider.address_region && `, ${provider.address_region}`}
                        </span>
                      </div>
                    )}
        ...
        {!loading && count > 0 && providers.length === 0 && (
          <div className="text-center py-16">
            <p className="text-charcoal-900">Les artisans sauvegardés ne sont plus disponibles.</p>
            <button
              type="button"
              onClick={clearFavorites}
