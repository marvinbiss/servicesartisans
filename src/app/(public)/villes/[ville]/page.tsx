...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!ville) {
    return { title: 'Ville non trouvée | ServicesArtisans' }
  }
  ...
  return {
    ...
    alternates: {
      canonical: `https://servicesartisans.fr/villes/${villeSlug}`,
    },
  }
}
...
async function getArtisansForCity(cityName: string) {
  try {
    ...
    const { data: providers } = await supabase
      .from('providers')
      .select('id, name, slug, specialty, rating_average, review_count, avatar_url, address_city')
      .eq('is_active', true)
          ...
          <section className="mb-16">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {artisans.slice(0, 3).map((artisan) => {
                const specialtySlug = (artisan.specialty || 'artisan').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
                return (
                  <Link
                    key={artisan.id}
                    href={`/services/${specialtySlug}/${villeSlug}/${artisan.slug}`}
                  ...
                  >
                    ...
                    {artisan.rating_average && artisan.rating_average > 0 && (
                        ...
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`w-4 h-4 ${
                                i < Math.round(artisan.rating_average || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                              }`}
                            />
                          ))}
                        </div>
