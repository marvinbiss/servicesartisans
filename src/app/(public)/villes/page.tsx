      ...
      <section className="relative bg-charcoal-950 text-white overflow-hidden">
        <div className="absolute inset-0">
          <div
            ...
            style={{
              background:
                'radial-gradient(ellipse 80% 50% at 50% -10%, rgba(232,107,75,0.18) 0%, transparent 60%), radial-gradient(ellipse 60% 60% at 80% 110%, rgba(61,139,104,0.12) 0%, transparent 50%)',
            }}
          />
          <div
        ...
        {sortedRegions.map(([region, regionVilles]) => {
          ...
          return (
            <details
              key={region}
              id={`region-${regionId}`}
              className="mb-4 bg-white rounded-2xl border border-sand-300 shadow-soft group"
            >
              <summary className="flex items-center gap-3 p-4 cursor-pointer list-none select-none hover:bg-sand-50 rounded-2xl transition-colors">
                ...
                <h3 className="font-heading text-base font-bold text-charcoal-900 tracking-tight">
                  {region}
                </h3>
                <span className="text-sm text-charcoal-500 font-medium">
                  ({regionVilles.length} villes)
                </span>
