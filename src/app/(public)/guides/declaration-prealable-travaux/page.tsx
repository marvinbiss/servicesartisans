  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-sand-50">
        ...
        {/* Hero */}
        <div className="bg-gradient-to-b from-green-50 to-sand-50">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {keyFigures.map((fig) => (
              <div
                key={fig.label}
                className="bg-white rounded-xl border border-sand-300 p-4 text-center"
              >
                <div
                  className={`inline-flex items-center justify-center w-10 h-10 rounded-lg ${fig.bg} mb-2`}
                >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          {/* Table of contents */}
          <nav className="bg-white rounded-xl border border-sand-300 p-6 mb-10">
            ...
            <ol className="space-y-2 text-sm">
              {[
                ...
                <li
                  key={i}
                  className="flex items-center gap-2 text-charcoal-600 hover:text-green-600"
                >
          ...
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {casConcernes.map((cas) => (
                <div key={cas.title} className={`bg-white rounded-xl border border-sand-300 p-5`}>
                  <div className="flex items-start gap-4">
                    <div
                      className={`flex-shrink-0 w-10 h-10 ${cas.color} rounded-lg flex items-center justify-center`}
                    >
                      <cas.icon className="w-5 h-5 text-charcoal-600" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-charcoal-900 mb-1">{cas.title}</h3>
                      <p className="text-sm text-charcoal-600">{cas.desc}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-white rounded-xl border border-sand-300 overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-sand-50 border-b border-sand-300">
                      <th className="text-left py-3 px-4 font-semibold text-charcoal-900">
                        {'Critère'}
                      </th>
                      <th className="text-left py-3 px-4 font-semibold text-charcoal-900">
                        {'Déclaration préalable'}
                  ...
                  <tbody className="divide-y divide-sand-200">
                    {[
                      ...
                      { critere: 'Délai d’instruction', dp: '1 mois', pc: '2 à 3 mois' },
                      {
                        critere: 'Formulaire',
                        dp: 'Cerfa 13703*10',
                        pc: 'Cerfa 13406*12 ou 13409*12',
                      },
                      {
                        critere: 'Accord tacite',
                        dp: 'Oui, après 1 mois sans réponse',
                        pc: 'Oui, après 2-3 mois sans réponse',
                      },
                      {
                        critere: 'Architecte obligatoire',
                        dp: 'Non',
                        pc: 'Oui si surface totale > 150 m²',
                      },
                      {
                        critere: 'Affichage sur le terrain',
                        dp: 'Oui, pendant 2 mois',
                        pc: 'Oui, pendant toute la durée du chantier',
                      },
                      {
                        critere: 'Recours des tiers',
                        dp: '2 mois après affichage',
                        pc: '2 mois après affichage',
                      },
                    ].map((row) => (
                      <tr key={row.critere} className="hover:bg-sand-50">
                        <td className="py-3 px-4 font-medium text-charcoal-900">{row.critere}</td>
                        <td className="py-3 px-4 text-charcoal-700">{row.dp}</td>
                        <td className="py-3 px-4 text-charcoal-700">{row.pc}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="p-4 bg-primary-50 border-t border-primary-200">
                <p className="text-sm text-primary-800">
                  ...
                  >
                    {'Consultez notre guide complet sur le permis de construire'}{' '}
            ...
            <div className="bg-white rounded-xl border border-sand-300 p-6">
              ...
              <div className="space-y-3">
                {[
                  {
                    code: 'DP1',
                    label: 'Plan de situation du terrain (toujours obligatoire)',
                    obligatoire: true,
                  },
                  {
                    code: 'DP2',
                  ...
                  },
                  {
                    code: 'DP3',
                    label:
                      'Plan en coupe du terrain et de la construction (si création de surface)',
                    obligatoire: false,
                  },
                  {
                    code: 'DP4',
                  ...
                  <div key={doc.code} className="flex items-start gap-3 p-3 bg-sand-50 rounded-lg">
                    ...
                    <span className="text-sm text-charcoal-700 flex-1">{doc.label}</span>
                    {doc.obligatoire && (
              ...
              <div className="space-y-6">
                ...
                <div className="p-4 bg-primary-50 rounded-lg">
                  <h3 className="font-semibold text-charcoal-900 mb-2">
                    {'Accord tacite : un droit fondamental'}
                  </h3>
                  ...
                  <ol className="space-y-2 text-sm text-charcoal-600">
                    <li className="flex items-start gap-2">
                      ...
                      <span>
                        {
                          'Envoyez une lettre recommandée à la mairie demandant une attestation de non-opposition.'
                        }
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                ...
                <div className="p-4 bg-amber-50 rounded-lg flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-charcoal-900 mb-1">
                      {'Attention : le retrait de la décision'}
                    </h3>
                    <p className="text-sm text-charcoal-600">
                      {
          ...
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {faqItems.map((faq, i) => (
                ...
                >
                  <summary className="flex items-center gap-3 p-5 cursor-pointer select-none hover:bg-sand-50 transition-colors">
                    <ChevronRight className="w-5 h-5 text-charcoal-400 group-open:rotate-90 transition-transform flex-shrink-0" />
                    <span className="font-medium text-charcoal-900">{faq.question}</span>
                  </summary>
                  <div className="px-5 pb-5 pl-13 text-sm text-charcoal-600 leading-relaxed">
                    {faq.answer}
                  </div>
                </details>
              ))}
          ...
          {/* Related guides */}
          <section>
            <h2 className="text-xl font-semibold text-charcoal-900 mb-4">{'Guides connexes'}</h2>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              >
                <h3 className="font-medium text-charcoal-900 group-hover:text-primary-500 transition-colors mb-1">
                  {'Permis de Construire 2026'}
                </h3>
                <p className="text-sm text-charcoal-500">
                  {'Quand est-il obligatoire ? Documents, délais et coûts.'}
                </p>
              </Link>
              <Link
              ...
              >
                ...
                <p className="text-sm text-charcoal-500">
                  {'Guide des normes électriques pour les particuliers.'}
                </p>
              </Link>
            </div>
          </section>
