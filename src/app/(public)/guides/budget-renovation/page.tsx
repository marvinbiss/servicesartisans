...
const fourchettesRenovation = [
  ...
  },
  {
    ...
    couleur: 'bg-primary-100 text-primary-600 border-primary-200',
    icon: Hammer,
    description:
...
const budgetParTravaux = [
  ...
  },
  {
    ...
    lignes: [
      ...
      {
        description: 'Pompe à chaleur air-air (climatisation réversible)',
        prix: '3 000 — 8 000 €',
      },
...
const simulateurLignes = [
  ...
  {
    poste: 'Fenêtres double vitrage (6 unités)',
    estimation: '2 400 — 5 400 €',
    aide: '240 — 600 €',
  },
...
const breadcrumbItems = [{ label: 'Guides', href: '/guides' }, { label: 'Budget rénovation' }]
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start gap-4">
                    <div className="flex items-center gap-4 md:w-1/3">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center ${item.couleur.split(' ')[0]}`}
                      >
                        <Icon className={`w-6 h-6 ${item.couleur.split(' ')[1]}`} />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-charcoal-900">{item.type}</h3>
                        <span
                          className={`inline-block mt-1 px-3 py-1 rounded-full text-sm font-bold border ${item.couleur}`}
                        >
                          {item.fourchette}
                        </span>
                      </div>
                    ...
                    <div className="md:w-2/3">
                      ...
                      <p className="text-sm text-charcoal-500 font-medium">{item.exemples}</p>
                    </div>
                  </div>
                </div>
              )
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-charcoal-900">{item.piece}</h3>
                  ...
                  <ul className="space-y-2">
                    {item.details.map((detail, index) => (
                      <li key={index} className="flex items-start gap-2">
                        ...
                        <span className="text-charcoal-600 text-sm">{detail}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 p-6 pb-4">
                    ...
                    <h3 className="text-lg font-bold text-charcoal-900">{categorie.travaux}</h3>
                  </div>
                  <div className="px-6 pb-6">
                    <table className="w-full">
                      <tbody>
                        {categorie.lignes.map((ligne, index) => (
                          <tr key={index} className={index % 2 === 0 ? 'bg-sand-50' : ''}>
                            <td className="py-2.5 px-3 text-charcoal-700 text-sm rounded-l-lg">
                              {ligne.description}
                            </td>
                            <td className="py-2.5 px-3 text-right font-semibold text-charcoal-900 text-sm whitespace-nowrap rounded-r-lg">
                              {ligne.prix}
                            </td>
                          </tr>
                        ))}
        ...
        <section className="max-w-5xl mx-auto px-4 py-10">
          ...
          <div className="space-y-4">
            {conseilsBudget.map((conseil, index) => (
              <div key={index} className="bg-white rounded-xl shadow-sm border border-sand-200 p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-charcoal-900 mb-1">{conseil.titre}</h3>
                    <p className="text-charcoal-600">{conseil.description}</p>
                  </div>
                </div>
              </div>
            ))}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-sand-50 border-b border-sand-300">
                    <th className="text-left py-4 px-6 text-sm font-bold text-charcoal-700">
                      Poste de travaux
                    </th>
                    <th className="text-right py-4 px-6 text-sm font-bold text-charcoal-700">
                      Estimation TTC
                    </th>
                    <th className="text-right py-4 px-6 text-sm font-bold text-charcoal-700">
                      Aides estimées
                ...
                <tbody>
                  {simulateurLignes.map((ligne, index) => (
                    <tr
                      key={index}
                      className={`border-b border-sand-100 ${index % 2 === 0 ? 'bg-white' : 'bg-sand-50/50'}`}
                    >
                      <td className="py-3.5 px-6 text-charcoal-700 text-sm">{ligne.poste}</td>
                      <td className="py-3.5 px-6 text-right text-sm font-medium text-charcoal-900">
                        {ligne.estimation}
                      </td>
                      <td className="py-3.5 px-6 text-right text-sm font-medium text-green-600">
                        {ligne.aide}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            >
              ...
              <p className="text-sm text-charcoal-500">
                {"MaPrimeRénov', CEE, éco-PTZ : le guide complet."}
              </p>
            </Link>
            <Link
            ...
            >
              ...
              <p className="text-sm text-charcoal-500">
                {'Comment comparer et négocier les devis.'}
              </p>
            </Link>
            <Link
