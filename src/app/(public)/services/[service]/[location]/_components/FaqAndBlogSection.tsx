...
export default function FaqAndBlogSection({ combinedFaq, service, location, serviceSlug }: Props) {
  ...
  const isBatiment = ['peintre-en-batiment', 'macon', 'couvreur', 'carreleur', 'menuisier'].includes(serviceSlug)
...
function BlogLink({ href, emoji, title, desc }: { href: string; emoji: string; title: string; desc: string }) {
  return (
    ...
    >
      ...
      <div>
        <span className="font-semibold text-gray-900 group-hover:text-blue-700 transition-colors">{title}</span>
