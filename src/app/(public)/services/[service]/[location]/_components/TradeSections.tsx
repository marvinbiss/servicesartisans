          ...
          <div className="mt-6 space-y-4">
            {trade.certifications && trade.certifications.length > 0 && (
              <div className="bg-accent-50 border border-accent-100 rounded-xl p-4">
                <h3 className="font-heading font-semibold text-accent-900 mb-2">Certifications et garanties à {location.name}</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {trade.certifications.map((cert, i) => (
                    <span key={i} className="text-sm bg-white text-accent-700 px-3 py-1 rounded-full border border-accent-200">
                      {cert}
                    </span>
                  ))}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-2xl shadow-soft border border-sand-200 p-8">
            ...
            <p className="text-charcoal-600 mb-6 text-sm pl-[calc(1rem+4px)]">
              Tarif horaire moyen à {location.name} : <strong className="text-charcoal-900">{Math.round(trade.priceRange.min * pricingMultiplier)}–{Math.round(trade.priceRange.max * pricingMultiplier)} {trade.priceRange.unit}</strong>.
