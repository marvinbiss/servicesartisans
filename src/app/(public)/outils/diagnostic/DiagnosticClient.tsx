  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="w-full bg-sand-300 rounded-full h-2.5">
          <div
      ...
      >
        ...
        {step === 1 && (
          <div>
            <h2 className="text-2xl font-bold text-charcoal-900 mb-2 font-heading">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {diagnosticCategories.map((category) => (
                <button
                  ...
                  className="group relative flex items-center gap-4 p-5 bg-white border-2 border-sand-300 rounded-xl hover:border-primary-400 hover:shadow-lg transition-all duration-200 text-left"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="block text-sm text-charcoal-500 mt-0.5">
                      {category.subProblems.length} problèmes courants
                    </span>
                  </div>
                  <ChevronRight className="w-5 h-5 text-sand-500 group-hover:text-primary-400 transition-colors flex-shrink-0" />
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 2 && selectedCategory && (
          <div>
            ...
            <p className="text-charcoal-600 mb-6">
              <span className="inline-flex items-center gap-1.5 text-sm bg-sand-100 px-3 py-1 rounded-full mr-2">
            ...
            <div className="grid grid-cols-1 gap-3">
              {selectedCategory.subProblems.map((problem) => (
                <button
                  ...
                  className="group flex items-start gap-4 p-5 bg-white border-2 border-sand-300 rounded-xl hover:border-primary-400 hover:shadow-lg transition-all duration-200 text-left"
                >
                  <div className="min-w-0 flex-1">
                    <span className="font-semibold text-charcoal-900 group-hover:text-primary-600 transition-colors">
                      {problem.label}
                    </span>
                    <span className="block text-sm text-charcoal-500 mt-1">
                      {problem.description}
                    </span>
                  ...
                  <ChevronRight className="w-5 h-5 text-sand-500 group-hover:text-primary-400 transition-colors flex-shrink-0 mt-1" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button
                ...
                className="group flex flex-col items-center gap-3 p-6 bg-white border-2 border-sand-300 rounded-xl hover:border-red-500 hover:shadow-lg transition-all duration-200"
              >
                ...
                <div className="text-center">
                  <span className="font-semibold text-charcoal-900 group-hover:text-red-700 transition-colors block">
                    Oui, c'est urgent
                  </span>
              ...
              <button
                ...
                className="group flex flex-col items-center gap-3 p-6 bg-white border-2 border-sand-300 rounded-xl hover:border-green-500 hover:shadow-lg transition-all duration-200"
              >
                ...
                <div className="text-center">
                  <span className="font-semibold text-charcoal-900 group-hover:text-green-700 transition-colors block">
                    Non, je planifie
                  </span>
                  <span className="text-sm text-charcoal-500 mt-1 block">
          ...
          <div>
            <div className="bg-gradient-to-br from-primary-50 to-indigo-50 border border-primary-200 rounded-2xl p-6 sm:p-8 mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm text-primary-500 font-medium">Notre recommandation</p>
              ...
              <p className="text-charcoal-700 mb-4">
                Pour votre problème de <strong>{selectedProblem.label.toLowerCase()}</strong>, un{' '}
                <strong>{serviceLabel.toLowerCase()}</strong> est le professionnel le plus adapté.
              </p>
              ...
              <Link
                href={getResultLink()}
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white font-semibold rounded-xl hover:from-primary-600 hover:to-primary-700 transition-all shadow-md hover:shadow-lg transform hover:scale-[1.02] w-full sm:w-auto justify-center"
              >
                {isUrgent ? (
                  <>
            ...
            {!isUrgent && (
              <div className="bg-white border border-sand-300 rounded-xl p-5 mb-6">
                <div className="flex items-center gap-2 mb-3">
                  <MapPin className="w-5 h-5 text-charcoal-500" />
                  <span className="font-semibold text-charcoal-900">
                    Précisez votre ville (optionnel)
                  </span>
                </div>
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 border border-sand-400 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-primary-400 outline-none transition-all text-sm"
                  />
                  {city.trim() && (
                    <Link
                      href={getResultLink()}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-primary-500 text-white font-medium rounded-lg hover:bg-primary-600 transition-colors text-sm"
                    >
            ...
            {selectedProblem.alternativeServices &&
              selectedProblem.alternativeServices.length > 0 && (
                <div className="bg-white border border-sand-300 rounded-xl p-5 mb-6">
                  <p className="font-semibold text-charcoal-900 mb-3">Alternatives possibles</p>
                  <div className="flex flex-wrap gap-2">
                    {selectedProblem.alternativeServices.map((altSlug) => (
                      <Link
                        key={altSlug}
                        href={`/services/${altSlug}`}
                        className="inline-flex items-center gap-2 px-4 py-2 bg-sand-50 border border-sand-300 rounded-lg hover:bg-primary-50 hover:border-primary-300 transition-all text-sm"
                      >
                        <span role="img" aria-hidden="true">
                          {serviceIcons[altSlug] || '🔧'}
                        </span>
            ...
            <div className="bg-white border border-sand-300 rounded-xl p-5 mb-6">
              <h3 className="font-semibold text-charcoal-900 mb-3">
                Conseils pour bien choisir votre artisan
              </h3>
              <ul className="space-y-2">
                ...
                <li className="flex items-start gap-2 text-sm text-charcoal-700">
                  <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  Assurez-vous que l'artisan possède une assurance responsabilité civile
                  professionnelle.
                </li>
                <li className="flex items-start gap-2 text-sm text-charcoal-700">
                  <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
            ...
            <div className="text-center">
              <button
                onClick={handleReset}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm text-charcoal-600 hover:text-primary-500 transition-colors"
              >
                <ArrowLeft className="w-4 h-4" />
