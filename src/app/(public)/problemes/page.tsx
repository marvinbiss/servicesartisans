    ...
    images: [
      {
        ...
        width: 1200,
        height: 630,
        alt: 'ServicesArtisans — Problèmes courants',
      },
    ],
  },
  twitter: {
...
const howSteps = [
  {
    ...
    description:
      'Parcourez notre liste de problèmes courants ou décrivez votre situation pour trouver le diagnostic adapté.',
  },
  {
    step: 2,
    icon: CheckCircle,
    ...
    description:
      'Consultez les gestes d’urgence, les symptômes à surveiller et les actions à mener en attendant l’artisan.',
  },
  {
    step: 3,
    icon: Wrench,
...
const faqItems = [
  {
    ...
    answer:
      'Parcourez notre liste de problèmes classés par catégorie (plomberie, électricité, serrurerie…). Chaque fiche détaille les symptômes typiques pour vous aider à identifier votre situation. En cas de doute, contactez un artisan pour un diagnostic professionnel.',
  },
  {
    question: 'Que faire en attendant l’artisan ?',
    answer:
      'Chaque fiche problème liste les gestes d’urgence à réaliser immédiatement : couper l’eau, l’électricité ou le gaz si nécessaire, sécuriser la zone et protéger vos biens. Ces actions simples limitent les dégâts en attendant l’intervention.',
  },
  {
    question: 'Les diagnostics sont-ils gratuits ?',
    answer:
      'Les fiches de diagnostic sur ServicesArtisans sont entièrement gratuites. La demande de devis est également gratuite et sans engagement. Seule l’intervention de l’artisan est payante, après acceptation de votre part.',
  },
  {
    question: 'Comment trouver un artisan pour mon problème ?',
    answer:
      'Depuis chaque fiche problème, vous pouvez accéder directement à la page du service concerné et demander un devis gratuit. Nos artisans sont référencés par SIREN et leurs coordonnées sont vérifiées.',
  },
  {
    question: 'Les coûts indiqués sont-ils fiables ?',
    answer:
      'Les fourchettes de prix sont des estimations indicatives basées sur les tarifs constatés en France. Le coût réel dépend de la complexité du problème, de votre région et de l’urgence. Seul un devis personnalisé fait foi.',
  },
]
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-charcoal-900 max-w-lg mx-auto">
              Sélectionnez la catégorie correspondante pour accéder au diagnostic détaillé et aux
              solutions.
            </p>
          </div>
              ...
              return (
                <div key={category.slug}>
                  <div className="flex items-center gap-3 mb-6">
                    <div
                      className={`w-10 h-10 ${category.bg} rounded-xl flex items-center justify-center`}
                    >
                      <Icon className={`w-5 h-5 ${category.color}`} />
                    </div>
                    <h3 className="text-xl font-bold text-charcoal-900">{category.name}</h3>
                    <span className="text-sm text-charcoal-400">
                      ({categoryProblems.length} problème{categoryProblems.length > 1 ? 's' : ''})
                    </span>
                  </div>
                    ...
                    {categoryProblems.map((problem) => {
                      ...
                      return (
                        <Link
                          ...
                          className="bg-white rounded-xl border border-sand-300 p-5 hover:shadow-lg hover:border-sand-400 transition-all group"
                        >
                          <div className="flex items-start justify-between mb-3">
                            <h4 className="font-semibold text-charcoal-900 group-hover:text-amber-600 transition-colors">
                              {problem.name}
                            </h4>
                            <span
                              className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${urgency.color}`}
                            >
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-10 relative">
            <div className="hidden md:block absolute top-14 left-[20%] right-[20%]">
              <div className="h-px border-t-2 border-dashed border-sand-300" />
            </div>
            {howSteps.map((item) => {
              ...
              return (
                <div key={item.step} className="relative text-center">
                  <div className="relative z-10 mx-auto mb-6">
                    ...
                    <div className="absolute -top-2 -right-2 w-7 h-7 bg-white border-2 border-sand-300 rounded-full flex items-center justify-center shadow-sm">
                      <span className="text-xs font-bold text-charcoal-700">{item.step}</span>
                    </div>
                  </div>
                  <h3 className="font-heading text-lg font-bold text-charcoal-900 mb-2">
                    {item.title}
                  </h3>
                  <p className="text-charcoal-900 text-sm leading-relaxed max-w-xs mx-auto">
                    {item.description}
                  </p>
                </div>
              )
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {faqItems.map((item) => (
              <details
                key={item.question}
                className="group bg-sand-50 rounded-xl border border-sand-200 overflow-hidden"
              >
                <summary className="flex items-center justify-between cursor-pointer px-6 py-5 text-left hover:bg-sand-100 transition-colors [&::-webkit-details-marker]:hidden">
                  <span className="font-semibold text-charcoal-900 pr-4">{item.question}</span>
                  <ChevronDown className="w-5 h-5 text-charcoal-400 shrink-0 group-open:rotate-180 transition-transform duration-200" />
                </summary>
                <div className="px-6 pb-5 text-charcoal-900 leading-relaxed text-sm">
                  {item.answer}
                </div>
              </details>
            ))}
      ...
      <section className="py-12 bg-sand-50 border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              <h3 className="font-semibold text-charcoal-900 mb-3">Services d'urgence</h3>
              <div className="space-y-2">
                ...
                <Link
                  href="/urgence/plombier"
                  className="block text-sm text-charcoal-600 hover:text-amber-600 py-1 transition-colors"
                >
                  Plombier urgence
                </Link>
                <Link
                  href="/urgence/electricien"
                  className="block text-sm text-charcoal-600 hover:text-amber-600 py-1 transition-colors"
                >
                  Électricien urgence
                </Link>
            ...
            <div>
              <h3 className="font-semibold text-charcoal-900 mb-3">Devis par métier</h3>
              <div className="space-y-2">
                <Link
                  href="/devis/plombier"
                  className="block text-sm text-charcoal-600 hover:text-amber-600 py-1 transition-colors"
            ...
            <div>
              <h3 className="font-semibold text-charcoal-900 mb-3">Outils utiles</h3>
              <div className="space-y-2">
                <Link
                  href="/outils"
                  className="block text-sm text-charcoal-600 hover:text-amber-600 py-1 transition-colors"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-sand-50 rounded-2xl border border-charcoal-200 p-6">
            ...
            <p className="text-xs text-charcoal-900 leading-relaxed">
              Les coûts et délais indiqués sont des estimations moyennes constatées en France
              métropolitaine. Ils varient selon la complexité du problème, votre région et
