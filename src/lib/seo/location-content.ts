...
import {
  getQuartierData,
  type QuartierProfile as QuartierDataProfile,
} from '@/lib/data/quartier-data'
import {
  getDeptPreposition,
  getDeptArticle,
...
function villeToPartialCommuneData(ville: Ville): CommuneData {
  ...
  const climatZone = mountainDepts.includes(ville.departementCode)
    ? 'montagnard'
    : regionClimate === 'oceanique'
      ? 'océanique'
      : regionClimate === 'continental'
        ? 'continental'
        : regionClimate === 'mediterraneen'
          ? 'méditerranéen'
          : regionClimate === 'montagnard'
            ? 'montagnard'
            : regionClimate === 'tropical'
              ? 'tropical'
              : 'semi-océanique'
  ...
  const CLIMATE_ESTIMATES: Record<
    string,
    {
      jours_gel: number
      precip: number
  ...
  const DEPT_FROST_ADJUST: Record<string, number> = {
    ...
    '62': 1.15,
    '80': 1.1,
    '02': 1.15, // Hauts-de-France: colder
    '67': 1.2,
    '68': 1.25,
    '57': 1.15,
    '88': 1.3, // Grand Est: colder, Vosges
    '25': 1.3,
    '39': 1.2,
    '70': 1.15, // Franche-Comté: Jura
    '63': 1.15,
    '15': 1.2,
    '43': 1.15, // Auvergne: Massif Central
    '48': 1.25,
    '12': 1.1,
    '09': 1.2,
    '66': 1.1, // Occitanie montagneux
    '06': 0.85,
    '83': 0.85,
    '13': 0.9, // Côte d'Azur: plus doux
    '33': 0.9,
    '40': 0.9,
    '64': 0.95, // Aquitaine: doux
    '29': 0.85,
    '56': 0.85,
    '22': 0.9,
    '35': 0.95, // Bretagne: doux
    '2A': 0.8,
    '2B': 0.85, // Corse
    '73': 1.4,
    '74': 1.35,
  ...
  const climEst = climEstBase
    ? {
        ...
        temp_hiver:
          Math.round(
            (climEstBase.temp_hiver + (frostAdj > 1 ? -(frostAdj - 1) * 3 : (1 - frostAdj) * 2)) *
              perturbation *
              10
          ) / 10,
        temp_ete:
          Math.round(
            (climEstBase.temp_ete + (frostAdj > 1 ? -(frostAdj - 1) * 1.5 : (1 - frostAdj) * 2)) *
              10
          ) / 10,
  ...
  const estimatedArtisans =
    deptCounts && pop > 0 ? Math.max(1, Math.round(deptCounts.artisans * cityShare)) : null
  const estimatedBtp =
    deptCounts && estimatedArtisans ? Math.max(1, Math.round(deptCounts.btp * cityShare)) : null
  ...
  const estimatedPassoiresDpe =
    realDpe != null ? realDpe : baseDpe != null ? Math.round(baseDpe * perturbation) : null
  ...
  return {
    ...
    revenu_median: REGIONAL_REVENU_MEDIAN[ville.region]
      ? Math.round(
          (REGIONAL_REVENU_MEDIAN[ville.region] *
            (pop >= 100000 ? 1.08 : pop >= 30000 ? 1.02 : pop >= 10000 ? 0.97 : 0.93) *
            perturbation) /
            10
        ) * 10
      : null,
    prix_m2_moyen:
      getVilleRealPrix(ville.slug) ??
...
function deriveQuartierCommuneData(
  ...
  return {
    ...
    nb_artisans_btp: cityData.nb_artisans_btp
      ...
      : null,
    part_maisons_pct:
      cityData.part_maisons_pct != null
        ? Math.max(
            5,
            Math.min(
              95,
              Math.round(
                (cityData.part_maisons_pct + (DENSITY_MAISONS_ADJUST[density] || 0)) * qPerturbation
              )
            )
          )
        : null,
    pct_passoires_dpe:
      cityData.pct_passoires_dpe != null
        ? Math.max(
            3,
            Math.min(55, Math.round(cityData.pct_passoires_dpe * dpeMult * qPerturbation))
          )
        : null,
...
const PRICING_NOTE_TEMPLATES: ((p: PricingNoteParams) => string)[] = [
  (p) =>
    `À ${p.name}, les tarifs d'un ${p.svc} se situent en moyenne entre ${p.min} et ${p.max} ${p.unit}, des prix ${p.label}. ${getRegionPreposition(p.region)}, le coût de la vie et la densité de professionnels influencent directement les tarifs pratiqués. Comparez au moins trois devis de ${p.svc}s à ${p.name} (${p.cp}) pour choisir la meilleure offre.`,
  (p) =>
    `Le tarif moyen d'un ${p.svc} à ${p.name} (${p.dept}) oscille entre ${p.min} et ${p.max} ${p.unit}. Ces prix, ${p.label}, reflètent la réalité du marché ${getRegionPreposition(p.region)}. Le coût final dépend de la nature de l'intervention, des matériaux et de l'urgence du chantier. Un devis détaillé reste la meilleure manière de budgétiser vos travaux.`,
  (p) =>
    `Comptez entre ${p.min} et ${p.max} ${p.unit} pour un ${p.svc} à ${p.name}, un tarif ${p.label}. La fourchette varie selon la complexité du chantier et les matériaux utilisés. À ${p.name} (${p.cp}), la concurrence entre professionnels vous permet de négocier en comparant plusieurs propositions.`,
  (p) =>
    `${getRegionPreposition(p.region)}, faire appel à un ${p.svc} à ${p.name} coûte en moyenne de ${p.min} à ${p.max} ${p.unit}. Ce tarif, ${p.label}, inclut le déplacement et la main-d'œuvre mais pas les fournitures spécifiques. Demandez un devis précis mentionnant le détail des postes pour éviter les mauvaises surprises.`,
  (p) =>
    `Les ${p.svc}s intervenant à ${p.name} (${p.dept}, ${p.cp}) facturent généralement entre ${p.min} et ${p.max} ${p.unit}. Ce niveau de prix est ${p.label}. Pour obtenir le meilleur rapport qualité-prix, nous conseillons de comparer les devis en vérifiant les garanties, assurances et délais de chaque professionnel.`,
]

function generatePricingNote(serviceSlug: string, serviceName: string, ville: Ville): string {
...
function generateLocalTips(serviceSlug: string, serviceName: string, ville: Ville): string[] {
  const trade = getTradeContent(serviceSlug)
...
function generateQuartierText(serviceName: string, ville: Ville): string {
  const svcLower = serviceName.toLowerCase()
...
function generateConclusion(
  ...
  const templates = [
    ...
    `N'attendez plus pour lancer votre projet : consultez dès maintenant les ${countSpace}${svcLower}s disponibles à ${ville.name} et dans le ${ville.departementCode}. ServicesArtisans vous donne accès aux coordonnées de professionnels vérifiés ${getRegionPreposition(ville.region)}, pour des devis rapides et sans engagement.${urgencyLine}${certLine}`,

    // 6 — Résumé des avantages
    `Annuaire vérifié, données SIREN officielles, accès gratuit : ServicesArtisans réunit toutes les conditions pour vous aider à trouver un ${svcLower} de confiance à ${ville.name} (${ville.codePostal}). Comparez les ${countSpace}professionnels ${getDeptArticle(ville.departement)} et contactez celui qui correspond à votre besoin.${urgencyLine}${certLine}`,

    // 7 — Proximité et réactivité
    `À ${ville.name}, un ${svcLower} qualifié n'est qu'à quelques clics. Parcourez les ${countSpace}profils référencés ${getDeptPreposition(ville.departement)} (${ville.region}) sur ServicesArtisans et trouvez le professionnel idéal pour votre projet, qu'il s'agisse d'un dépannage ou de travaux planifiés.${urgencyLine}${certLine}`,
...
const SVC_LOCATION_FAQ_POOL: {
  q: (p: SvcLocFaqParams) => string
  a: (p: SvcLocFaqParams) => string
}[] = [
  {
  ...
  },
  {
    ...
    a: (p) =>
      `Sur ServicesArtisans, les ${p.svc}s référencés à ${p.name} sont vérifiés via leur numéro SIREN. Comparez les profils, vérifiez les qualifications et demandez un devis gratuit.`,
  },
  {
    q: (p) => `Un ${p.svc} à ${p.name} intervient-il en urgence ?`,
    a: (p) =>
      `De nombreux ${p.svc}s à ${p.name} proposent des interventions d'urgence, y compris le week-end et les jours fériés. Consultez notre page urgence pour un dépannage rapide dans le ${p.deptCode}.`,
  },
  {
    q: (p) => `Quels sont les délais d'intervention d'un ${p.svc} à ${p.name} ?`,
    a: (p) =>
      `En fonction de la demande, un ${p.svc} à ${p.name} peut intervenir sous 24 à 48h pour un rendez-vous planifié, et dans l'heure pour une urgence. La disponibilité dépend de la saison et de la zone.`,
  },
  {
    q: (p) => `Le ${p.svc} à ${p.name} est-il assuré ?`,
    a: (p) =>
      `Les ${p.svc}s du bâtiment à ${p.name} sont tenus de souscrire une assurance décennale. Demandez systématiquement l'attestation avant le début des travaux pour vous protéger.`,
  },
  {
    q: (p) => `Faut-il un devis pour des travaux de ${p.svc} à ${p.name} ?`,
    a: (p) =>
      `Oui, un devis écrit est obligatoire pour tout travail de ${p.svc} dépassant 150 €. À ${p.name}, demandez au moins 3 devis pour comparer. Notre service de mise en relation est 100% gratuit.`,
  },
  {
    q: (p) => `Quelles garanties offre un ${p.svc} à ${p.name} ?`,
    a: (p) =>
      `Un ${p.svc} à ${p.name} doit fournir : garantie décennale (10 ans), garantie de parfait achèvement (1 an) et garantie biennale (2 ans) sur les équipements. Vérifiez ces points avant de signer.`,
  },
  {
    q: (p) => `Le climat ${p.climate.toLowerCase()} à ${p.name} impacte-t-il les travaux ?`,
    a: (p) =>
      `Oui, le climat ${p.climate.toLowerCase()} de ${p.name} influence directement les besoins en ${p.svc}. Les professionnels locaux connaissent ces contraintes et adaptent leurs prestations en conséquence.`,
  },
  {
    q: (p) => `Quels quartiers de ${p.name} sont couverts par un ${p.svc} ?`,
    a: (p) =>
      `Les ${p.svc}s référencés sur ServicesArtisans couvrent l'ensemble des quartiers de ${p.name} et ses environs ${getDeptPreposition(p.dept)} (${p.deptCode}). Consultez la section quartiers sur cette page.`,
  },
  {
    q: (p) => `Un ${p.svc} à ${p.name} peut-il m'aider pour une rénovation énergétique ?`,
    a: (p) =>
      `Selon le corps de métier, un ${p.svc} à ${p.name} peut contribuer à la rénovation énergétique de votre logement. Pour accéder aux aides (MaPrimeRénov', CEE), choisissez un artisan RGE.`,
  },
  {
    q: (p) => `Comment comparer les ${p.svc}s à ${p.name} ?`,
    a: (p) =>
      `Comparez les devis en tenant compte du prix, des délais, des garanties et de l'expérience locale. À ${p.name}, privilégiez les ${p.svc}s qui connaissent le bâti typique ${getRegionArticle(p.region)}.`,
  },
  {
    q: (p) => `Peut-on faire intervenir un ${p.svc} le week-end à ${p.name} ?`,
    a: (p) =>
      `Certains ${p.svc}s à ${p.name} proposent des interventions le week-end, notamment pour les urgences. Un supplément de 20 à 50% est généralement appliqué hors heures ouvrées.`,
  },
  {
    q: (p) => `Quelles aides pour des travaux de ${p.svc} à ${p.name} ?`,
    a: (p) =>
      `Les habitants de ${p.name} (${p.dept}) peuvent bénéficier de MaPrimeRénov', de l'éco-PTZ et des CEE pour certains travaux de ${p.svc}. Renseignez-vous auprès de l'ANAH ou de votre mairie.`,
  },
  {
    q: (p) => `Que faire en cas de litige avec un ${p.svc} à ${p.name} ?`,
    a: (p) =>
      `En cas de problème, adressez une réclamation écrite au ${p.svc}. Si le litige persiste, contactez le médiateur de la consommation ou la DGCCRF ${getDeptArticle(p.dept)}. L'assurance décennale couvre les malfaçons.`,
  },
  {
    q: (p) => `Les ${p.svc}s à ${p.name} sont-ils référencés officiellement ?`,
    a: (p) =>
      `Oui, tous les ${p.svc}s listés sur ServicesArtisans à ${p.name} sont identifiés par leur numéro SIREN via les données officielles du gouvernement. C'est un gage de transparence et de fiabilité.`,
  },
]
...
const CLIMATE_SERVICE_TIPS: Record<ClimateZone, ((svc: string, name: string) => string)[]> = {
  oceanique: [
    (svc, name) =>
      `Le climat océanique de ${name} soumet le bâti à une humidité persistante. Un ${svc} intervenant régulièrement dans la zone connaît ces contraintes et adapte ses matériaux et techniques pour garantir la durabilité de ses interventions.`,
    (svc, name) =>
      `À ${name}, l'air marin et les précipitations fréquentes exigent des choix de matériaux adaptés. Votre ${svc} doit privilégier des solutions résistantes à l'humidité pour un résultat durable.`,
    (svc, name) =>
      `Les façades et installations extérieures à ${name} subissent l'usure du climat océanique. Les ${svc}s expérimentés dans la zone sélectionnent des matériaux anticorrosion et des traitements hydrofuges spécifiques.`,
    (svc, name) =>
      `L'humidité récurrente à ${name} nécessite une attention particulière lors des travaux. Un ${svc} local sait que la ventilation et l'étanchéité sont les priorités dans un environnement océanique.`,
    (svc, name) =>
      `Le vent et la pluie à ${name} accélèrent le vieillissement du bâti. Un ${svc} qualifié tiendra compte de ces facteurs climatiques dans ses recommandations et son choix de matériaux.`,
  ],
  continental: [
    (svc, name) =>
      `À ${name}, les hivers rigoureux et les étés chauds créent d'importants écarts thermiques. Un ${svc} compétent dimensionne ses interventions en tenant compte de ces contraintes continentales pour éviter les désordres liés aux dilatations.`,
    (svc, name) =>
      `Le climat continental de ${name} soumet les installations à des cycles gel-dégel répétés. Les ${svc}s locaux maîtrisent les techniques de protection adaptées à ces conditions exigeantes.`,
    (svc, name) =>
      `Les températures négatives hivernales à ${name} requièrent des précautions spécifiques. Un ${svc} expérimenté prévoit l'isolation et la protection antigel adaptées au climat continental.`,
    (svc, name) =>
      `En été comme en hiver, le bâti à ${name} est soumis à rude épreuve. Votre ${svc} doit anticiper les contraintes thermiques propres au climat continental pour garantir la pérennité de ses travaux.`,
    (svc, name) =>
      `Le gel prolongé à ${name} peut endommager les installations mal protégées. Un ${svc} connaissant la région sait quels matériaux et techniques résistent aux hivers continentaux.`,
  ],
  mediterraneen: [
    (svc, name) =>
      `À ${name}, le soleil intense et les épisodes de pluies violentes alternent tout au long de l'année. Un ${svc} local adapte ses interventions à ce climat méditerranéen pour assurer leur longévité.`,
    (svc, name) =>
      `La chaleur estivale à ${name} et les orages d'automne sollicitent fortement le bâti. Les ${svc}s expérimentés en zone méditerranéenne choisissent des matériaux résistants aux UV et aux intempéries.`,
    (svc, name) =>
      `Le retrait des argiles en période de sécheresse à ${name} peut affecter les fondations. Un ${svc} qualifié prend en compte ces mouvements de terrain typiques du climat méditerranéen.`,
    (svc, name) =>
      `Les températures élevées à ${name} engendrent des contraintes spécifiques sur les matériaux. Votre ${svc} doit sélectionner des produits résistants à la chaleur et aux rayons UV pour des travaux durables.`,
    (svc, name) =>
      `Les épisodes cévenols à ${name} peuvent causer des infiltrations soudaines. Un ${svc} local anticipe ces risques en privilégiant l'étanchéité renforcée dans ses interventions.`,
  ],
  montagnard: [
    (svc, name) =>
      `En altitude à ${name}, le poids de la neige, le gel prolongé et les écarts thermiques imposent des contraintes majeures. Un ${svc} rodé au climat montagnard utilise des matériaux et techniques adaptés à ces conditions extrêmes.`,
    (svc, name) =>
      `Les conditions hivernales rigoureuses à ${name} limitent la période de travaux extérieurs. Les ${svc}s expérimentés en montagne planifient les chantiers en conséquence et renforcent les protections antigel.`,
    (svc, name) =>
      `À ${name}, l'isolation thermique est un enjeu central. Un ${svc} connaissant le bâti montagnard sait que chaque intervention doit contribuer à la performance énergétique du logement.`,
    (svc, name) =>
      `Le gel intense et la neige à ${name} sollicitent l'ensemble du bâti. Votre ${svc} doit maîtriser les techniques spécifiques au climat de montagne pour garantir des interventions résistantes aux intempéries.`,
    (svc, name) =>
      `Les contraintes d'accès en hiver à ${name} allongent les délais d'intervention. Un ${svc} local anticipe ces difficultés et propose des solutions adaptées à la saisonnalité montagnarde.`,
  ],
  'semi-oceanique': [
    (svc, name) =>
      `Le climat semi-océanique de ${name} combine humidité modérée et hivers frais. Un ${svc} intervenant dans cette zone adapte ses recommandations pour garantir une bonne ventilation et une protection durable contre l'humidité.`,
    (svc, name) =>
      `À ${name}, les précipitations régulières mais modérées sollicitent le bâti de manière continue. Les ${svc}s locaux connaissent l'importance des traitements préventifs dans ce climat tempéré humide.`,
    (svc, name) =>
      `Le climat tempéré de ${name} favorise la condensation dans les logements mal ventilés. Un ${svc} compétent intègre la gestion de l'humidité dans chacune de ses interventions.`,
    (svc, name) =>
      `Les hivers frais et les étés doux à ${name} créent un environnement favorable aux moisissures. Votre ${svc} doit veiller à la ventilation et à l'étanchéité pour préserver la salubrité du logement.`,
    (svc, name) =>
      `À ${name}, le climat semi-océanique préserve relativement le bâti mais demande un entretien régulier. Un ${svc} professionnel recommandera un calendrier d'entretien adapté à ces conditions.`,
  ],
  tropical: [
    (svc, name) =>
      `Le climat tropical de ${name} soumet le bâti à une humidité constante et des températures élevées. Un ${svc} expérimenté en zone tropicale sélectionne des matériaux résistants aux moisissures et à la corrosion.`,
    (svc, name) =>
      `À ${name}, les risques cycloniques exigent des normes de construction renforcées. Les ${svc}s locaux maîtrisent les techniques paracycloniques pour garantir la résistance des installations.`,
    (svc, name) =>
      `L'air salin et l'humidité permanente à ${name} accélèrent la dégradation des matériaux. Un ${svc} connaissant le climat tropical choisit des produits spécifiquement conçus pour résister à ces conditions.`,
    (svc, name) =>
      `Les termites et insectes xylophages sont un fléau à ${name}. Votre ${svc} doit intégrer la protection contre ces nuisibles dans ses interventions, notamment pour les structures en bois.`,
    (svc, name) =>
      `La chaleur et l'humidité constantes à ${name} rendent indispensable une climatisation performante. Un ${svc} local sait dimensionner les installations pour ce climat exigeant.`,
  ],
}

function generateServiceClimateTip(
  svc: string,
  cityName: string,
  climate: ClimateZone,
  seed: number
...
export function generateLocationContent(
  ...
  const mountainDepts = ['73', '74', '05', '38', '09', '65', '04']
  const finalClimate = mountainDepts.includes(ville.departementCode)
    ? CLIMATES.find((c) => c.key === 'montagnard') || climate
    : climate
  ...
  const faqParams: SvcLocFaqParams = {
    svc: svcLower,
    name: ville.name,
    dept: ville.departement,
  ...
  const dataDriven = generateDataDrivenContent(
    effectiveCommuneData,
    serviceSlug,
    serviceName,
    providerCount
  )
  ...
  return {
    introText:
      dataDriven?.intro || generateIntroText(serviceSlug, serviceName, ville, providerCount),
...
const SERVICE_PRIORITY: Record<BuildingEra, string[]> = {
  ...
  // Pivot full RGE 2026-05-03 : serrurier/carreleur/vitrier retirés (commodity
  // hors RGE) — remplacés par salle-de-bain (intent rénovation pièces humides
  // RGE-canonical) et isolation-thermique pour les épochs concernées.
  '1980-2000': [
    'peintre-en-batiment',
    'menuisier',
    'chauffagiste',
...
const QUARTIER_INTROS: IntroFn[] = [
  (q, v, dep, code, pop, era, arch, density) =>
    `Le quartier ${q} à ${v} (${code}) se caractérise par un ${era.toLowerCase()} en ${density.toLowerCase()}. ${arch} Avec ${pop} habitants, ${v} ${getDeptPreposition(dep)} dispose d'artisans qualifiés pour ce type de bâti.`,
  (q, v, dep, code, pop, era, arch, density) =>
  ...
  (q, v, dep, code, pop, era, arch, density) =>
    `Situé à ${v} ${getDeptPreposition(dep)} (${code}), le quartier ${q} accueille un parc immobilier de ${era.toLowerCase()} en ${density.toLowerCase()}. ${arch} Pour répondre aux besoins des ${pop} habitants, notre annuaire recense les artisans dont le savoir-faire correspond au bâti du secteur.`,
  (q, v, dep, code, pop, era, arch, density) =>
  ...
  (q, v, dep, code, pop, era, arch, density) =>
    `${q} fait partie des quartiers de ${v} (${code}) où le ${era.toLowerCase()} définit le paysage urbain en ${density.toLowerCase()}. ${arch} Pour les ${pop} habitants ${getDeptArticle(dep)}, notre plateforme facilite la mise en relation avec des artisans compétents sur ce type de bâti.`,
]
...
const BATIMENT_CONTEXTS: Record<BuildingEra, ((q: string, v: string) => string)[]> = {
  ...
  haussmannien: [
    (q, v) =>
      `Le quartier ${q} à ${v} abrite un patrimoine haussmannien d'exception exigeant des artisans maîtrisant la restauration. Parquet en point de Hongrie, moulures, cheminées en marbre : chaque élément raconte l'histoire du bâtiment. La rénovation doit préserver ce cachet tout en intégrant le confort moderne. Seuls des artisans expérimentés à ${v} peuvent relever ce défi.`,
    (q, v) =>
...
const ERA_TIPS: Record<BuildingEra, ((q: string, v: string) => string)[]> = {
  'pre-1950': [
    (q, v) =>
      `Si vous habitez un logement ancien à ${q}, privilégiez un diagnostic complet avant d'engager des travaux. Vérifiez canalisations (plomb), installation électrique et charpente. À ${v}, les artisans spécialisés en bâti ancien vous conseillent sur les priorités et les aides disponibles (MaPrimeRénov', éco-PTZ). Demandez toujours plusieurs devis.`,
    (q, v) =>
      `Si votre logement est ancien à ${q}, respectez l'ordre logique : gros œuvre (toiture, murs) d'abord, puis réseaux (électricité, plomberie, chauffage), enfin finitions (peinture, sols). À ${v}, les artisans expérimentés planifient pour éviter de reprendre des travaux déjà réalisés. Cette approche vous fait gagner du temps et de l'argent.`,
    (q, v) =>
      `Dans un logement d'avant 1950 à ${q}, l'humidité est souvent le premier ennemi. Avant toute rénovation esthétique, traitez les remontées capillaires et assurez une ventilation correcte. Les artisans de ${v} spécialisés en bâti ancien sauront poser un diagnostic d'humidité et proposer des solutions pérennes adaptées aux murs anciens.`,
    (q, v) =>
      `Rénover un logement ancien à ${q} offre l'opportunité de valoriser des éléments de caractère : tomettes, poutres apparentes, cheminées en pierre. À ${v}, les artisans formés à la restauration du patrimoine savent allier authenticité et confort moderne. Pensez à vérifier l'éligibilité de vos travaux aux aides de l'ANAH.`,
  ],
  '1950-1980': [
    (q, v) =>
      `Si vous habitez un logement 1950-1980 à ${q}, faites réaliser un diagnostic amiante et un DPE avant tout chantier. Ces documents orienteront vos travaux prioritaires. À ${v}, les artisans certifiés savent manipuler les matériaux amiantés en sécurité. La rénovation énergétique ouvre droit à des aides substantielles.`,
    (q, v) =>
      `Si vous habitez un logement d'après-guerre à ${q}, la priorité est souvent la performance énergétique. Isolation par l'extérieur, remplacement des fenêtres et chauffage performant peuvent diviser par deux votre facture. Les artisans RGE de ${v} vous permettent de bénéficier des aides financières. Faites un audit énergétique pour définir le plan optimal.`,
    (q, v) =>
  ...
  'post-2000': [
    (q, v) =>
      `Votre logement récent à ${q} est aux normes mais peut gagner en confort. Pensez à la climatisation réversible, la domotique pour optimiser votre consommation, ou un aménagement sur mesure (dressing, verrière). Les artisans de ${v} spécialisés en finitions haut de gamme sauront valoriser votre intérieur.`,
    (q, v) =>
      `Dans un logement post-2000 à ${q}, les travaux sont liés à l'évolution de vos besoins : bureau, borne de recharge, agrandissement. À ${v}, les artisans intervenant sur le bâti récent connaissent les matériaux et techniques actuels. Vérifiez les garanties constructeur encore en cours avant certains travaux.`,
    (q, v) =>
      `Votre résidence récente à ${q} bénéficie d'une bonne isolation de base, mais la domotique peut encore optimiser votre consommation. Thermostat connecté, volets automatisés, éclairage programmable : les artisans de ${v} spécialisés en maison intelligente transforment votre quotidien sans gros travaux.`,
    (q, v) =>
      `Dans les constructions post-2000 à ${q}, l'aménagement extérieur est souvent le levier de valorisation le plus efficace. Pergola bioclimatique, terrasse en bois composite, éclairage paysager : les artisans de ${v} créent des espaces de vie prolongés qui augmentent la valeur de votre bien.`,
  ],
  haussmannien: [
    (q, v) =>
      `Si vous rénovez un appartement haussmannien à ${q}, choisissez des artisans expérimentés en restauration. Parquet ancien, moulures, cheminées exigent un savoir-faire spécifique. À ${v}, vérifiez si votre immeuble est en secteur protégé (ABF) : certains travaux de façade peuvent nécessiter une autorisation préalable.`,
    (q, v) =>
      `Un haussmannien à ${q} offre des volumes exceptionnels. Les artisans de ${v} peuvent moderniser électricité et plomberie de manière invisible, installer une cuisine contemporaine dans le respect des proportions, et restaurer les éléments de décor (parquet, moulures, cheminées) qui font le charme de ces logements.`,
    (q, v) =>
      `La rénovation d'un haussmannien à ${q} passe souvent par la mise aux normes du réseau électrique, dissimulé dans les moulures et les plinthes pour préserver l'esthétique. Les artisans de ${v} spécialisés en patrimoine maîtrisent ces techniques d'encastrement invisible qui respectent le cachet de votre appartement.`,
    (q, v) =>
      `Les parquets en point de Hongrie ou en bâtons rompus de votre haussmannien à ${q} méritent une attention particulière. Ponçage, vitrification ou huilage : chaque finition a ses avantages. Les parqueteurs de ${v} évaluent l'épaisseur restante de bois noble avant d'intervenir pour garantir la longévité du plancher.`,
  ],
  mixte: [
    (q, v) =>
      `Dans un quartier au bâti mixte comme ${q}, commencez par identifier l'époque de construction pour cibler les travaux pertinents. Un artisan polyvalent de ${v} vous orientera vers les interventions prioritaires après un état des lieux. Les techniques varient considérablement entre bâtiment ancien et résidence récente.`,
    (q, v) =>
      `À ${q}, la diversité du bâti implique de choisir des artisans adaptés à votre situation. Que vous habitiez un immeuble ancien ou une résidence moderne à ${v}, demandez un diagnostic précis avant d'engager des travaux. Les devis doivent être établis après visite sur site pour prendre en compte les particularités de votre construction.`,
    (q, v) =>
      `Le bâti hétérogène de ${q} signifie que deux logements voisins peuvent avoir des besoins radicalement différents. Avant d'engager des travaux à ${v}, faites établir un état des lieux technique par un artisan qualifié. Cette étape évite les mauvaises surprises et permet de budgétiser avec précision.`,
    (q, v) =>
      `Dans un quartier mixte comme ${q}, profitez de la variété des chantiers pour trouver des artisans polyvalents à ${v}. Ces professionnels, habitués à passer du bâti ancien à la construction récente, apportent une vision globale précieuse. Ils sauront vous conseiller sur les matériaux et techniques les mieux adaptés à votre logement.`,
  ],
}

// 15 FAQ templates — 4 selected per page via hash
const FAQ_POOL: {
  q: (n: string, v: string) => string
  a: (n: string, v: string, dep: string, code: string, era: string, issues: string[]) => string
}[] = [
  {
    q: (n, v) => `Quels artisans interviennent à ${n}, ${v} ?`,
    a: (n, v, dep, code) =>
      `Notre annuaire référence des artisans dans plus de 40 corps de métier intervenant dans le quartier ${n} à ${v} (${code}) : plombiers, électriciens, serruriers, chauffagistes, peintres, menuisiers, couvreurs, maçons et plus. Tous sont identifiés à partir des données SIREN ${getDeptArticle(dep)}.`,
  },
  {
    q: (n) => `Comment obtenir un devis gratuit à ${n} ?`,
    a: (n, v) =>
      `Sélectionnez le service souhaité, indiquez ${v} comme localisation, et décrivez votre projet. Vous recevrez jusqu'à 3 propositions d'artisans qualifiés intervenant à ${n}. Service 100% gratuit et sans engagement.`,
  },
  {
    q: (_n, v) => `Combien coûte un artisan à ${v} ?`,
    a: (_n, v, _dep, code) =>
      `Les tarifs varient selon le corps de métier, la complexité et l'urgence. À ${v} (${code}), comptez en moyenne 50–90 €/h selon la spécialité. Demandez plusieurs devis pour comparer les prix et prestations.`,
  },
  {
    q: (n) => `Quel est le délai d'intervention moyen à ${n} ?`,
    a: (n, v) =>
      `Pour une urgence (fuite, panne, serrure bloquée), les artisans de ${v} interviennent sous 1 à 4 heures à ${n}. Pour des travaux planifiés, comptez 1 à 3 semaines selon la disponibilité et l'ampleur du chantier.`,
  },
  {
  ...
  },
  {
    ...
    a: (n, v, _dep, _code, era, issues) =>
      `À ${n} (${v}), le bâti de type ${era.toLowerCase()} génère des besoins spécifiques : ${issues.slice(0, 3).join(', ').toLowerCase()}. Les artisans locaux connaissent ces problématiques et proposent des solutions adaptées.`,
  },
  {
    q: (n, v) => `Comment choisir le bon artisan à ${n}, ${v} ?`,
    a: (n, v, _dep, _code, era) =>
      `Pour bien choisir un artisan à ${n}, vérifiez que sa spécialité correspond à votre besoin. Le bâti de ${v} étant de type ${era.toLowerCase()}, privilégiez un professionnel expérimenté sur cette construction. Comparez au moins 3 devis, vérifiez les assurances (décennale, RC pro), et consultez les avis.`,
  },
  {
    q: () => `Les devis sont-ils vraiment gratuits et sans engagement ?`,
    a: (_n, v) =>
      `Oui, les devis demandés via notre plateforme sont entièrement gratuits et sans engagement. Vous pouvez recevoir jusqu'à 3 propositions d'artisans de ${v}, les comparer, et choisir librement. Aucun frais caché, aucune obligation.`,
  },
  {
    q: (n) => `Un artisan peut-il intervenir en urgence à ${n} ?`,
    a: (n, v) =>
      `Certains artisans de ${v} proposent des interventions d'urgence à ${n} : fuite d'eau, panne électrique, porte claquée, panne de chauffage. Disponibilité et délais variables selon les professionnels. Les tarifs d'urgence incluent généralement un supplément.`,
  },
  {
    q: (n) => `Quelles garanties pour les travaux à ${n} ?`,
    a: (n, v) =>
      `Les artisans professionnels à ${v} sont couverts par l'assurance décennale et la RC professionnelle. Pour les travaux à ${n}, exigez une attestation d'assurance à jour. La garantie de parfait achèvement (1 an) et la garantie biennale (2 ans) complètent votre protection.`,
  },
  {
    q: (n, v) => `Faut-il un permis pour rénover à ${n}, ${v} ?`,
    a: (n, v, dep) =>
      `Cela dépend de la nature des travaux. À ${n} (${v}), les travaux intérieurs ne nécessitent généralement pas d'autorisation. En revanche, modification de façade, extension ou changement de destination requiert une déclaration préalable auprès de la mairie. Consultez le PLU ${getDeptArticle(dep)}.`,
  },
  {
    q: (_n, v) => `Comment comparer efficacement les artisans à ${v} ?`,
    a: (_n, v) =>
      `Demandez au minimum 3 devis détaillés pour le même périmètre de travaux à ${v}. Vérifiez que chaque devis précise : nature des travaux, matériaux, délais, prix HT et TTC, conditions de paiement. Consultez les avis clients et vérifiez les certifications (RGE, Qualibat).`,
  },
  {
    q: (_n, v) => `Quelles aides financières pour rénover à ${v} ?`,
    a: (_n, v, dep, code) =>
      `Les habitants de ${v} (${code}) peuvent bénéficier de : MaPrimeRénov', éco-PTZ, CEE (Certificats d'Économies d'Énergie), et parfois des aides locales ${getDeptArticle(dep)}. Pour en bénéficier, les travaux doivent être réalisés par un artisan certifié RGE. Demandez un audit énergétique.`,
  },
  {
    q: (n) => `Le bâti à ${n} nécessite-t-il des artisans spécialisés ?`,
    a: (n, v, _dep, _code, era, issues) =>
      `Le quartier ${n} à ${v} est composé de ${era.toLowerCase()}, ce qui implique des compétences spécifiques. Les problématiques courantes (${issues.slice(0, 2).join(', ').toLowerCase()}) demandent des artisans formés. Privilégiez des professionnels expérimentés sur des chantiers similaires.`,
  },
  {
    q: () => `Comment vérifier un artisan avant de l'engager ?`,
    a: (_n, v) =>
      `Avant d'engager un artisan à ${v} : vérifiez son inscription au registre des métiers (SIRET actif), ses assurances (décennale, RC pro), sa certification RGE si nécessaire. Demandez des références de chantiers similaires. Méfiez-vous des devis anormalement bas.`,
  },
]
...
const PROXIMITY_TEMPLATES: ((q: string, v: string, era: string) => string)[] = [
  (q, v, era) =>
    `Faire appel à un artisan proche de ${q} offre des avantages concrets : temps de déplacement réduit, connaissance du ${era.toLowerCase()}, réactivité en cas d'urgence, et suivi facilité. Un professionnel de ${v} connaît les réglementations locales et les fournisseurs du secteur.`,
  (q, v, era) =>
    `Choisir un artisan intervenant à ${q} vous garantit un service adapté au ${era.toLowerCase()} du quartier. Moins de frais de déplacement, meilleure connaissance du bâti local, délais plus courts : la proximité est un critère de qualité. Les artisans de ${v} s'engagent sur des délais réalistes.`,
  (q, v) =>
    `Un artisan basé à ${v} et intervenant à ${q} offre une réactivité précieuse. En cas d'urgence, il peut être sur place en moins d'une heure. Sa connaissance du quartier (accès, stationnement, bâtiments) lui permet d'intervenir efficacement. La proximité facilite aussi le suivi de chantier.`,
  (q, v, era) =>
    `La proximité d'un artisan est un atout majeur à ${q}. Un professionnel de ${v} habitué au ${era.toLowerCase()} anticipe les difficultés et connaît les bonnes pratiques locales. Le bouche-à-oreille fonctionne : un artisan local qui fait du bon travail est recommandé par vos voisins.`,
  (q, v, era) =>
    `Privilégier un artisan local pour vos travaux à ${q}, c'est aussi soutenir l'économie de proximité. Les professionnels de ${v} investissent dans la connaissance du ${era.toLowerCase()}, travaillent avec des fournisseurs locaux et s'engagent sur la qualité. Résultat : interventions soignées et tarifs compétitifs.`,
  (q, v) =>
    `Les artisans de ${v} intervenant à ${q} combinent expertise technique et connaissance du terrain. Ils savent quels matériaux conviennent, connaissent les contraintes d'accès et peuvent mobiliser leur réseau local (autres corps de métier, fournisseurs). Cette proximité se traduit par des chantiers mieux coordonnés.`,
  (q, v, era) =>
    `Pour des travaux sur du ${era.toLowerCase()} à ${q}, un artisan local est un choix judicieux. Il connaît les pathologies récurrentes du quartier, dispose de références vérifiables à ${v} et peut assurer un service après-vente réactif. La confiance se construit dans la durée grâce à cette proximité.`,
  (q, v, era) =>
    `Engager un artisan de ${v} pour intervenir à ${q}, c'est bénéficier d'un professionnel familier du ${era.toLowerCase()} environnant. Les frais de déplacement sont réduits, les délais d'intervention raccourcis, et le suivi après chantier simplifié. Autant d'arguments en faveur d'un prestataire de proximité.`,
  (q, v) =>
    `À ${q}, miser sur un artisan local de ${v} présente des avantages concrets au quotidien. En cas de malfaçon ou de retouche nécessaire, il se déplace sans délai ni surcoût. Sa réputation locale l'engage à fournir un travail irréprochable, car chaque chantier est une vitrine pour de futurs clients du quartier.`,
  (q, v, era) =>
    `Les professionnels installés à ${v} qui interviennent à ${q} ont développé une expertise du ${era.toLowerCase()} au fil de leurs chantiers. Cette expérience accumulée leur permet d'anticiper les imprévus, de proposer les solutions les plus adaptées et de respecter les délais annoncés.`,
  (q, v, era) =>
    `Travailler avec un artisan basé près de ${q} à ${v} facilite chaque étape du projet. Du premier rendez-vous au suivi post-chantier, la proximité géographique simplifie les échanges. Sur du ${era.toLowerCase()}, cette connaissance fine du bâti local est un avantage décisif pour la réussite de vos travaux.`,
  (q, v) =>
    `Un professionnel du bâtiment ancré à ${v} et habitué à intervenir à ${q} est un partenaire de confiance pour vos travaux. Il connaît les règles d'urbanisme locales, les spécificités du quartier et peut coordonner plusieurs corps de métier en s'appuyant sur son réseau de confrères à proximité.`,
]
...
function getQuartierProfile(ville: Ville, quartierName: string): QuartierProfile {
  ...
  if (ville.departementCode === '75') {
    ...
    const lyonEras: Record<string, BuildingEra> = {
      'Vieux Lyon': 'pre-1950',
      "Presqu'île": 'haussmannien',
      'Croix-Rousse': 'pre-1950',
      'Part-Dieu': '1980-2000',
      Confluence: 'post-2000',
      Gerland: '1950-1980',
      Villeurbanne: '1950-1980',
    }
    ...
    const marseilleEras: Record<string, BuildingEra> = {
      'Vieux-Port': 'pre-1950',
      'Le Panier': 'pre-1950',
      'La Joliette': '1980-2000',
      Castellane: '1950-1980',
      'La Canebière': 'haussmannien',
      Prado: 'haussmannien',
      Bonneveine: '1980-2000',
      'Les Calanques': 'post-2000',
    }
...
const ERA_CLIMAT_IMPACT: Record<string, ((q: string) => string)[]> = {
  'pre-1950': [
    (q) =>
      `Pour le bâti ancien de ${q}, le gel accentue les risques d'infiltration et de fissuration des maçonneries. L'isolation thermique est d'autant plus critique.`,
    (q) =>
      `À ${q}, les constructions d'avant 1950 sont vulnérables aux cycles gel-dégel qui fragilisent les joints de maçonnerie. Un entretien préventif régulier limite les dégradations.`,
    (q) =>
      `Le patrimoine bâti ancien de ${q} nécessite une attention particulière lors des épisodes de froid : les matériaux poreux absorbent l'humidité qui, en gelant, fissure pierres et enduits.`,
  ],
  '1950-1980': [
    (q) =>
      `Les constructions d'après-guerre de ${q}, souvent peu isolées, subissent de plein fouet les variations thermiques. Le remplacement des fenêtres simple vitrage est une priorité.`,
    (q) =>
      `À ${q}, les immeubles des années 1950-1980 présentent des façades exposées aux intempéries sans protection thermique adéquate. La priorité : isoler et remplacer les menuiseries.`,
    (q) =>
      `Le béton des constructions d'après-guerre de ${q} conduit le froid en profondeur. En période de gel, les ponts thermiques génèrent condensation et moisissures à l'intérieur.`,
  ],
  '1980-2000': [
    (q) =>
      `Les logements des années 1980-2000 à ${q} résistent mieux au climat grâce aux premières normes thermiques, mais les menuiseries d'origine arrivent en fin de vie.`,
    (q) =>
      `À ${q}, les résidences 1980-2000 sont mieux protégées contre le froid que le parc ancien, mais l'étanchéité des toitures-terrasses et des joints de façade mérite un contrôle régulier.`,
    (q) =>
      `Le bâti des années 1980-2000 de ${q} bénéficie d'une isolation correcte pour l'époque, mais le vieillissement des matériaux réduit leur efficacité face aux aléas climatiques.`,
  ],
  'post-2000': [
    (q) =>
      `Le bâti récent de ${q} est conçu pour résister aux aléas climatiques. L'entretien courant (VMC, étanchéité) reste néanmoins essentiel.`,
    (q) =>
      `Les constructions modernes de ${q} intègrent une isolation performante qui protège des variations saisonnières. Un suivi d'entretien préventif suffit généralement.`,
    (q) =>
      `À ${q}, les logements post-2000 supportent bien les contraintes climatiques grâce aux normes RT en vigueur. La VMC double flux et le double vitrage limitent les déperditions.`,
  ],
  haussmannien: [
    (q) =>
      `Les immeubles haussmanniens de ${q}, avec leurs murs épais en pierre, offrent une bonne inertie thermique, mais les jonctions fenêtres-maçonnerie sont des points faibles.`,
    (q) =>
      `À ${q}, le patrimoine haussmannien bénéficie de murs massifs qui régulent naturellement la température. En revanche, les courants d'air aux menuiseries anciennes restent un défi.`,
    (q) =>
      `La pierre de taille des immeubles haussmanniens de ${q} absorbe et restitue la chaleur, mais les planchers hauts et les grandes fenêtres augmentent les besoins de chauffage en hiver.`,
  ],
  mixte: [
    (q) =>
      `La diversité architecturale de ${q} implique des réponses climatiques variées, du traitement anti-humidité du bâti ancien à l'entretien préventif du bâti récent.`,
    (q) =>
      `À ${q}, chaque époque de construction réagit différemment aux aléas climatiques. Un diagnostic thermique adapté à chaque bâtiment est recommandé avant travaux.`,
    (q) =>
      `Le tissu urbain mixte de ${q} présente des vulnérabilités climatiques hétérogènes : le bâti ancien craint l'humidité, tandis que le bâti récent nécessite un entretien régulier.`,
  ],
}
...
function generateQuartierDataDrivenContent(
  ...
  const qData = deriveQuartierCommuneData(
    cityData,
    quartierName,
    ville.slug,
    profile.era,
  ...
  if (prixQ && prixCity) {
    ...
    const diffLabel =
      diff > 0 ? `${diff} % au-dessus` : diff < 0 ? `${Math.abs(diff)} % en dessous` : 'au niveau'
  ...
  if (dpe != null && cityDpe != null) {
    ...
    const eraNote =
      eraNotes[Math.abs(hashCode(`enr-${ville.slug}-${quartierName}`)) % eraNotes.length]
    const dpeMedianSuffix = dpeMedianLabel
      ? ` Le ${dpeMedianLabel} du quartier confirme cette tendance.`
      : ''
  ...
  if (gel != null || precip != null) {
    ...
    if (impactFns) {
      const impactFn =
        impactFns[Math.abs(hashCode(`clim-${ville.slug}-${quartierName}`)) % impactFns.length]
      climatParts.push(impactFn(quartierName))
    }

    if (trDebut != null && trFin != null) {
      climatParts.push(
        `La période optimale pour les travaux extérieurs à ${quartierName} s'étend de ${monthName(trDebut)} à ${monthName(trFin)}.`
      )
    }
  ...
  const periodeTravaux =
    trDebut != null && trFin != null ? `${monthName(trDebut)} à ${monthName(trFin)}` : null
...
export function generateQuartierContent(
  ville: Ville,
  quartierName: string,
  serviceSlug?: string
): QuartierContent {
  ...
  const intro = introFn(
    ...
    ville.population,
    profile.eraLabel,
    profile.architecturalNote,
    profile.densityLabel
  )

  // Building context — by era
  const ctxTemplates = BATIMENT_CONTEXTS[profile.era]
  const batimentContext = ctxTemplates[
    Math.abs(hashCode(`ctx-${ville.slug}-${quartierName}${seedSuffix}`)) % ctxTemplates.length
  ...
  const sdTemplates = [
    `Dans le quartier ${quartierName} à ${ville.name}, les services fréquemment demandés dans les quartiers à ${profile.eraLabel.toLowerCase()} sont : ${pricingLines.join(' · ')}. Ces tarifs indicatifs ${getRegionPreposition(ville.region)} varient selon la complexité, l'urgence et les matériaux. Comparez plusieurs devis.`,
    `À ${quartierName} (${ville.name}), les artisans les plus sollicités interviennent sur du ${profile.eraLabel.toLowerCase()}. Tarifs constatés ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Prix indicatifs, variables selon le chantier et les matériaux choisis.`,
    `Les habitants de ${quartierName} à ${ville.name} font appel à des artisans spécialisés dans le ${profile.eraLabel.toLowerCase()}. Fourchettes tarifaires ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Demandez plusieurs devis pour affiner votre budget.`,
    `Pour un logement à ${quartierName}, les travaux courants sur du ${profile.eraLabel.toLowerCase()} couvrent : ${pricingLines.join(' · ')}. Ces estimations ${getRegionPreposition(ville.region)} dépendent de l'état du bâti, de l'accès chantier et des finitions souhaitées.`,
    `Le quartier ${quartierName} à ${ville.name}, typique du ${profile.eraLabel.toLowerCase()}, génère une demande régulière pour : ${pricingLines.join(' · ')}. Tarifs indicatifs ${getRegionPreposition(ville.region)}, à confirmer par devis personnalisé.`,
    `Avec un parc immobilier de ${profile.eraLabel.toLowerCase()}, ${quartierName} concentre des besoins artisanaux précis. Les prestations les plus courantes ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Un devis sur site reste indispensable pour un chiffrage fiable.`,
    `Les résidents de ${quartierName} à ${ville.name} recherchent régulièrement des artisans pour du ${profile.eraLabel.toLowerCase()}. Grille tarifaire indicative ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Les prix définitifs dépendent de l'état du logement et de l'ampleur des travaux.`,
    `À ${ville.name}, le quartier ${quartierName} présente un bâti de ${profile.eraLabel.toLowerCase()} qui oriente la demande en artisanat. Principales prestations et tarifs constatés ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Sollicitez plusieurs professionnels pour comparer.`,
    `Le ${profile.eraLabel.toLowerCase()} caractéristique de ${quartierName} à ${ville.name} implique des interventions spécifiques. Estimations tarifaires ${getRegionPreposition(ville.region)} : ${pricingLines.join(' · ')}. Ces fourchettes sont données à titre indicatif et peuvent varier selon l'accès au chantier.`,
    `Quartier de ${ville.name} au bâti de ${profile.eraLabel.toLowerCase()}, ${quartierName} voit une demande soutenue pour : ${pricingLines.join(' · ')}. Ces tarifs moyens ${getRegionPreposition(ville.region)} constituent un repère utile, mais seul un devis après visite garantit un prix juste.`,
  ]
  const servicesDemandes =
    sdTemplates[
      Math.abs(hashCode(`sd-${ville.slug}-${quartierName}${seedSuffix}`)) % sdTemplates.length
    ]

  // Tips — by era
  const tipTemplates = ERA_TIPS[profile.era]
  const conseils = tipTemplates[
    Math.abs(hashCode(`tips-${ville.slug}-${quartierName}${seedSuffix}`)) % tipTemplates.length
  ](quartierName, ville.name)

  // Proximity
  const proxFn =
    PROXIMITY_TEMPLATES[
      Math.abs(hashCode(`prox-${ville.slug}-${quartierName}${seedSuffix}`)) %
        PROXIMITY_TEMPLATES.length
    ]
...
const DEPT_SERVICE_PRIORITY: Record<ClimateZone, string[]> = {
  // Pivot full RGE 2026-05-03 : serrurier/carreleur/vitrier retirés (commodity
  // hors RGE) — remplacés par salle-de-bain + isolation-thermique (climats
  // froids/humides) ou pompe-a-chaleur (climats tempérés / chauffage).
  oceanique: [
    'couvreur',
    'peintre-en-batiment',
    'plombier',
...
const DEPT_INTROS: DeptIntroFn[] = [
  (name, code, region, pop, chefLieu, climate, economy, housing) =>
    `Le ${name} (${code}), ${getRegionPreposition(region)}, abrite ${pop} habitants avec ${chefLieu} pour chef-lieu. Caractérisé par un ${climate.toLowerCase()} et une ${economy.toLowerCase()}, le département présente un parc immobilier de type ${housing.toLowerCase()}. Les artisans du ${code} adaptent leurs interventions à ces spécificités.`,
  (name, code, region, pop, _chefLieu, climate, economy, housing) =>
    `Trouver un artisan qualifié ${getDeptPreposition(name)} (${code}) nécessite de comprendre les particularités du département. ${getRegionPreposition(region)}, ce territoire de ${pop} habitants bénéficie d'un ${climate.toLowerCase()} influençant les besoins en artisanat. L'${economy.toLowerCase()} et le ${housing.toLowerCase()} orientent les interventions prioritaires.`,
  (name, code, region, pop, chefLieu, climate, economy, housing) =>
    `Le département ${getDeptArticle(name)} (${code}), dont le chef-lieu est ${chefLieu}, se situe ${getRegionPreposition(region)}. Avec ${pop} habitants, il conjugue ${climate.toLowerCase()} et ${economy.toLowerCase()}. Le ${housing.toLowerCase()} impose aux artisans locaux une expertise adaptée aux constructions dominantes.`,
  (name, code, region, pop, _chefLieu, climate, economy, housing) =>
    `Vous recherchez un artisan ${getDeptPreposition(name)} ? Ce département de ${pop} habitants ${getRegionPreposition(region)} (${code}) se distingue par son ${climate.toLowerCase()}, son ${economy.toLowerCase()} et un parc de ${housing.toLowerCase()}. Notre annuaire référence les professionnels correspondant à ces caractéristiques.`,
  ...
  (name, code, region, pop, chefLieu, climate, economy, housing) =>
    `Le ${name} (${code}), rattaché à la région ${region}, est un territoire de ${pop} habitants dont le chef-lieu est ${chefLieu}. Entre ${climate.toLowerCase()} et ${economy.toLowerCase()}, le département accueille un ${housing.toLowerCase()} aux besoins variés.`,
  (name, code, region, pop, _chefLieu, climate, economy, housing) =>
    `Avec ${pop} habitants ${getDeptPreposition(name)} (${code}), ce département ${getRegionArticle(region)} offre un marché de l'artisanat dynamique. Le ${climate.toLowerCase()} influence les chantiers, l'${economy.toLowerCase()} soutient la demande, et le ${housing.toLowerCase()} oriente les spécialités sollicitées.`,
  (name, code, region, pop, chefLieu, climate, _economy, housing) =>
    `Le ${name}, département ${code} ${getRegionArticle(region)}, regroupe ${pop} habitants avec ${chefLieu} comme préfecture. Le ${climate.toLowerCase()} impacte l'usure des bâtiments. Le ${housing.toLowerCase()} requiert des artisans formés aux techniques adaptées.`,
]

const DEPT_TIPS: Record<ClimateZone, ((name: string, code: string) => string)[]> = {
  oceanique: [
    (name, code) =>
      `${getDeptPreposition(name)} (${code}), l'humidité est l'ennemi principal du bâti. Privilégiez une VMC performante et des traitements anti-humidité réguliers. Les façades exposées à l'ouest subissent davantage d'intempéries — un ravalement tous les 10-15 ans est recommandé.`,
    (name) =>
      `Le climat océanique ${getDeptArticle(name)} appelle un entretien régulier des toitures (démoussage annuel), des gouttières (nettoyage biannuel) et des menuiseries. Investir dans des matériaux résistants à l'humidité réduit les coûts de maintenance à long terme.`,
    (name, code) =>
      `Les vents marins ${getDeptPreposition(name)} (${code}) chargent l'air de sel et d'humidité, accélérant la corrosion des métaux et le ternissement des façades. Optez pour des menuiseries aluminium laquées ou PVC, plus résistantes que le bois brut dans ce contexte.`,
    (name) =>
      `L'isolation par l'extérieur (ITE) est particulièrement recommandée ${getDeptPreposition(name)} en climat océanique : elle protège les murs des infiltrations tout en réduisant les ponts thermiques. Les artisans locaux maîtrisent la pose de bardages ventilés adaptés.`,
    (name, code) =>
      `${getDeptPreposition(name)} (${code}), les remontées capillaires dans les maisons anciennes sont fréquentes. Un diagnostic humidité avant travaux est essentiel : injection de résine hydrophobe, drainage périphérique ou cuvelage selon la gravité. Consultez un professionnel qualifié.`,
  ],
  continental: [
    (name, code) =>
      `Le climat continental ${getDeptArticle(name)} (${code}) impose des contraintes thermiques majeures. L'isolation des combles et murs est le premier investissement rentable — les économies de chauffage remboursent l'investissement en 5-8 ans. Protégez les canalisations extérieures contre le gel.`,
    (name) =>
      `${getDeptPreposition(name)}, les écarts de température entre été et hiver sollicitent les matériaux. Vérifiez joints de dilatation, enduits de façade et chauffage avant chaque hiver. Un entretien de chaudière annuel est obligatoire.`,
    (name, code) =>
      `Les gelées tardives ${getDeptPreposition(name)} (${code}) peuvent endommager les canalisations extérieures et les compteurs d'eau. Avant l'hiver, purgez les circuits non chauffés et isolez les tuyaux exposés avec des manchons calorifuges.`,
    (name) =>
      `L'amplitude thermique annuelle ${getDeptArticle(name)} (parfois 40°C entre hiver et été) commande le choix de matériaux à faible coefficient de dilatation. Les revêtements de sol, enduits et joints doivent absorber ces variations sans fissurer.`,
    (name, code) =>
      `${getDeptPreposition(name)} (${code}), la pompe à chaleur air-eau est le système de chauffage le plus recommandé : performante par grand froid grâce aux modèles basse température, elle permet des économies de 50-70% par rapport au fioul. Vérifiez le COP à -15°C avant d'investir.`,
  ],
  mediterraneen: [
    (name, code) =>
      `Le climat méditerranéen ${getDeptArticle(name)} (${code}) impose deux priorités : la protection contre la chaleur estivale (volets, isolation, climatisation) et la gestion des épisodes de pluies intenses. Les artisans locaux connaissent les matériaux adaptés au soleil intense.`,
    (name) =>
      `${getDeptPreposition(name)}, l'ensoleillement intense accélère le vieillissement des peintures et joints. Optez pour des revêtements haute résistance UV. Les épisodes cévenols exigent des gouttières surdimensionnées et une étanchéité irréprochable.`,
    (name, code) =>
      `La sécheresse estivale ${getDeptPreposition(name)} (${code}) provoque le retrait-gonflement des argiles, première cause de sinistres sur maisons individuelles. Un diagnostic géotechnique et des fondations adaptées sont indispensables pour construire ou rénover.`,
    (name) =>
      `L'isolation par l'extérieur en enduit clair est idéale ${getDeptPreposition(name)} : elle réduit la surchauffe estivale de 5 à 8°C et offre un confort d'été sans climatisation excessive. Combinée à des brise-soleil, c'est la solution la plus performante.`,
    (name, code) =>
      `Les terrasses et toitures-terrasses ${getDeptPreposition(name)} (${code}) nécessitent une étanchéité renforcée avec des membranes PVC ou EPDM résistantes aux UV. Prévoyez des évacuations surdimensionnées : les pluies méditerranéennes peuvent déverser 100 mm en une heure.`,
  ],
  montagnard: [
    (name, code) =>
      `En montagne ${getDeptPreposition(name)} (${code}), la surcharge de neige dimensionne la charpente et l'isolation doit être renforcée (R > 7 en combles). Planifiez vos travaux entre mai et octobre. Le bois de charpente doit être traité contre l'humidité.`,
    (name) =>
      `Le département ${getDeptArticle(name)} en altitude présente des contraintes spécifiques : matériaux résistants au gel, isolation thermique renforcée, chaudière performante. Les artisans locaux maîtrisent les normes de construction en zone montagne et les aides spécifiques.`,
    (name, code) =>
      `${getDeptPreposition(name)} (${code}), le déneigement des toitures est une obligation de sécurité au-delà de 40 cm d'accumulation. Prévoyez des crochets pare-neige et des gouttières chauffantes pour éviter les dégâts liés aux redoux brutaux.`,
    (name) =>
      `Le triple vitrage est rentable ${getDeptPreposition(name)} en zone montagnarde : le surcoût de 15-20% par rapport au double vitrage est amorti en 6-8 ans par les économies de chauffage. Associez-le à des volets bois pour une isolation maximale.`,
    (name, code) =>
      `Les fondations ${getDeptPreposition(name)} (${code}) doivent descendre sous la profondeur de gel (80 cm à 1,20 m selon l'altitude). Un vide sanitaire ventilé protège la dalle contre l'humidité du sol. Choisissez un artisan habitué aux normes montagne.`,
  ],
  'semi-oceanique': [
    (name, code) =>
      `Le climat tempéré ${getDeptArticle(name)} (${code}) facilite les travaux toute l'année, avec une attention particulière en hiver pour le chauffage et la ventilation. L'humidité modérée nécessite une VMC efficace. Les artisans connaissent les spécificités locales.`,
    (name) =>
      `${getDeptPreposition(name)}, le climat semi-océanique offre des conditions favorables à la plupart des chantiers. Les périodes de gel obligent à reporter la maçonnerie extérieure. Profitez du printemps et de l'automne pour les ravalements.`,
    (name, code) =>
      `L'humidité modérée ${getDeptArticle(name)} (${code}) favorise le développement de mousses sur les toitures et façades nord. Un traitement préventif biannuel au printemps et à l'automne évite les dégradations coûteuses à long terme.`,
    (name) =>
      `${getDeptPreposition(name)}, la VMC hygroréglable de type B est le meilleur compromis : elle adapte le débit aux besoins réels de chaque pièce, réduisant les déperditions thermiques de 10-15% par rapport à une VMC autoréglable.`,
    (name, code) =>
      `Le département ${getDeptArticle(name)} (${code}) bénéficie de conditions idéales pour la rénovation énergétique globale : isolation + ventilation + chauffage. Les aides MaPrimeRénov' en rénovation d'ampleur couvrent jusqu'à 90% des travaux pour les ménages modestes.`,
  ],
  tropical: [
    (name, code) =>
      `Le climat tropical ${getDeptArticle(name)} (${code}) impose des normes spécifiques : résistance aux cyclones, protection anti-termites, ventilation naturelle renforcée. Faites vérifier la structure de votre habitation avant la saison cyclonique.`,
    (name) =>
      `${getDeptPreposition(name)}, l'humidité permanente et la chaleur accélèrent le vieillissement des constructions. Traitements anti-corrosion, peintures fongicides, bois traité classe 4 : les matériaux doivent être adaptés au climat tropical.`,
    (name, code) =>
      `Les termites sont un fléau majeur ${getDeptPreposition(name)} (${code}). Le diagnostic termites est obligatoire avant toute vente, et un traitement préventif du bâti (barrière chimique ou pièges) protège votre investissement pour 10 ans.`,
    (name) =>
      `La ventilation naturelle traversante est le premier levier de confort ${getDeptPreposition(name)}. Avant d'investir dans la climatisation, assurez-vous que votre habitation bénéficie d'ouvertures opposées et d'un bon brassage d'air.`,
    (name, code) =>
      `En zone cyclonique ${getDeptPreposition(name)} (${code}), les menuiseries doivent résister à des vents de 250 km/h. Volets cycloniques, ancrages renforcés et toiture solidarisée : faites vérifier votre habitation par un professionnel agréé avant chaque saison.`,
  ],
}

const DEPT_FAQ_POOL: {
  q: (name: string, code: string) => string
  a: (
    name: string,
    code: string,
    region: string,
  ...
  },
  {
    ...
    a: (name, _code, _region, _pop, climate, issues) =>
      `Le ${climate.toLowerCase()} ${getDeptArticle(name)} génère des besoins spécifiques : ${issues.slice(0, 3).join(', ').toLowerCase()}. Les artisans locaux proposent des solutions éprouvées.`,
  },
  {
    q: (name, code) => `Comment obtenir un devis gratuit ${getDeptPreposition(name)} (${code}) ?`,
    a: (name) =>
      `Choisissez le service, indiquez votre ville ${getDeptPreposition(name)}, et décrivez votre projet. Vous recevrez jusqu'à 3 propositions d'artisans qualifiés. Service 100% gratuit et sans engagement.`,
  },
  {
    q: (name) => `Les artisans ${getDeptArticle(name)} sont-ils assurés ?`,
    a: (name, _code, region) =>
      `Les artisans ${getDeptArticle(name)} (${region}) sont tenus de disposer d'une assurance décennale et d'une RC pro. Exigez une attestation à jour avant les travaux.`,
  },
  {
    q: (name) => `Quel est le délai moyen pour des travaux ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `${getDeptPreposition(name)}, comptez 1-4h pour une urgence, 1-3 semaines pour des travaux planifiés, et 2-4 mois pour une rénovation complète.`,
  },
  {
    q: (name) => `Quelles aides financières sont disponibles ${getDeptPreposition(name)} ?`,
    a: (name, code, region) =>
      `Les habitants ${getDeptArticle(name)} (${code}) peuvent bénéficier de MaPrimeRénov', éco-PTZ, CEE, et aides locales ${getRegionArticle(region)}. Travaux réalisés par un artisan RGE.`,
  },
  {
    q: (name) => `Comment le climat influence-t-il les travaux ${getDeptPreposition(name)} ?`,
    a: (name, _code, _region, _pop, climate, issues) =>
      `Le ${climate.toLowerCase()} ${getDeptArticle(name)} impacte le bâti : ${issues.slice(0, 2).join(' ; ').toLowerCase()}. Les artisans adaptent techniques et matériaux à ces contraintes.`,
  },
  {
    q: (name) => `Peut-on trouver un artisan en urgence ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `Certains artisans ${getDeptArticle(name)} proposent des interventions d'urgence pour les situations critiques : fuite, panne électrique, serrure bloquée, chauffage défaillant. Les délais varient selon les disponibilités et la localisation.`,
  },
  {
    q: (name) => `Quel type de bâti domine ${getDeptPreposition(name)} ?`,
    a: (name, _code, _region, _pop, _climate, issues) =>
      `Le parc ${getDeptArticle(name)} est varié, avec des problématiques récurrentes : ${issues.slice(0, 3).join(', ').toLowerCase()}. L'importance de choisir un artisan expérimenté localement.`,
  },
  {
    q: (name) => `Comment vérifier un artisan ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `Vérifiez son SIRET actif, demandez assurance décennale et RC pro, consultez les avis, exigez un devis détaillé signé. Les certifiés RGE ou Qualibat offrent des garanties supplémentaires ${getDeptPreposition(name)}.`,
  },
  {
    q: (name) => `Combien coûte un artisan ${getDeptPreposition(name)} ?`,
    a: (name, code, region) =>
      `${getDeptPreposition(name)} (${code}), comptez 45-90 €/h selon la spécialité. ${getRegionPreposition(region)}, demandez plusieurs devis pour comparer. Tarifs incluant généralement le déplacement.`,
  },
  {
    q: (name) => `Faut-il des autorisations pour rénover ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `Les travaux intérieurs ne nécessitent généralement aucune autorisation ${getDeptPreposition(name)}. Les modifications de façade ou extensions exigent une déclaration préalable. Consultez le PLU.`,
  },
  {
    q: () => `Les devis sont-ils gratuits et sans engagement ?`,
    a: (name) =>
      `Oui, les devis via ServicesArtisans pour le département ${getDeptArticle(name)} sont gratuits et sans engagement. Comparez jusqu'à 3 propositions de professionnels qualifiés.`,
  },
  {
    q: (name) => `Quelles certifications rechercher ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `${getDeptPreposition(name)}, privilégiez : RGE (aides rénovation), Qualibat (qualification), QualiPV (solaire), QualiBois et QualiPAC (chauffage). Vérifiez la validité sur les annuaires officiels.`,
  },
  {
    q: (name) => `Comment planifier une rénovation ${getDeptPreposition(name)} ?`,
    a: (name) =>
      `Pour rénover ${getDeptPreposition(name)}, commencez par un audit énergétique. Définissez les priorités : gros œuvre, réseaux, isolation, finitions. Un maître d'œuvre peut coordonner (8-12% du montant).`,
  },
]

// Real economy mapping by department code (based on actual French economic data)
const TERTIAIRE_DEPTS = new Set([
  '75',
  '92',
  '93',
  '94',
...
const INDUSTRIEL_DEPTS = new Set([
  '08',
  '10',
  '25',
  '39',
...
const TOURISTIQUE_DEPTS = new Set([
  '2A',
  '2B',
  '73',
  '74',
...
const ANCIEN_PIERRE_DEPTS = new Set([
  '24',
  '46',
  '12',
  '19',
...
const APRES_GUERRE_DEPTS = new Set([
  '59',
  '62',
  '93',
  '94',
...
const MIXTE_URBAIN_DEPTS = new Set([
  '75',
  '92',
  '69',
  '13',
...
function getDepartementProfile(dept: import('@/lib/data/france').Departement): DepartementProfile {
  const climateKey =
    DEPT_CLIMATE_OVERRIDES[dept.code] || REGION_CLIMATE[dept.region] || 'semi-oceanique'
  ...
  while (selectedIssues.length < 5 && selectedIssues.length < allIssues.length) {
    ...
    if (!used.has(idx)) {
      used.add(idx)
      selectedIssues.push(allIssues[idx])
    }
    s = Math.abs(hashCode(`di${s}-${selectedIssues.length}`))
  }

  return {
    climate: finalClimate.key,
    climateLabel: finalClimate.label,
    economy: economy.key,
...
export function generateDepartementContent(
  dept: import('@/lib/data/france').Departement
): DepartementContent {
  ...
  const intro = introFn(
    ...
    dept.population,
    dept.chefLieu,
    profile.climateLabel,
    profile.economyLabel,
    profile.housingLabel
  )
  ...
  const conseilsDepartement = tipTemplates[
    Math.abs(hashCode(`tips-dept-${dept.slug}`)) % tipTemplates.length
  ](dept.name, dept.code)
  ...
  const faqItems = faqIndices.map((idx) => {
    ...
    return {
      question: f.q(dept.name, dept.code),
      answer: f.a(
        dept.name,
        dept.code,
        dept.region,
...
const REGION_GEO: Record<string, GeoType> = {
  'ile-de-france': 'plaine',
  bretagne: 'littoral',
  normandie: 'littoral',
  'pays-de-la-loire': 'littoral',
  'hauts-de-france': 'plaine',
  'grand-est': 'plaine',
  'bourgogne-franche-comte': 'mixte-geo',
  'centre-val-de-loire': 'plaine',
  'provence-alpes-cote-d-azur': 'mixte-geo',
  occitanie: 'mixte-geo',
  corse: 'insulaire',
  'auvergne-rhone-alpes': 'montagne',
  'nouvelle-aquitaine': 'littoral',
  guadeloupe: 'insulaire',
  martinique: 'insulaire',
  guyane: 'plaine',
  'la-reunion': 'insulaire',
  mayotte: 'insulaire',
}

const REGION_ECONOMY: Record<string, RegionalEconomy> = {
  'ile-de-france': 'metropole-services',
  bretagne: 'agriculture-viticulture',
  normandie: 'industrie-reconversion',
  'pays-de-la-loire': 'economie-diversifiee',
  'hauts-de-france': 'industrie-reconversion',
  'grand-est': 'industrie-reconversion',
  'bourgogne-franche-comte': 'agriculture-viticulture',
  'centre-val-de-loire': 'agriculture-viticulture',
  'provence-alpes-cote-d-azur': 'tourisme-patrimoine',
  occitanie: 'tourisme-patrimoine',
  corse: 'tourisme-patrimoine',
  'auvergne-rhone-alpes': 'economie-diversifiee',
  'nouvelle-aquitaine': 'economie-diversifiee',
  guadeloupe: 'tourisme-patrimoine',
  martinique: 'tourisme-patrimoine',
  guyane: 'economie-diversifiee',
  'la-reunion': 'tourisme-patrimoine',
  mayotte: 'economie-diversifiee',
}
...
const REGION_CONTEXTS: Record<
  RegionalEconomy,
  ((name: string, deptCount: number, cityCount: number) => string)[]
> = {
  'metropole-services': [
    (name, deptCount, cityCount) =>
      `La région ${name}, pôle tertiaire majeur, regroupe ${deptCount} départements et ${cityCount} villes. L'activité économique soutenue génère une forte demande en artisanat : rénovation d'appartements, aménagement de bureaux, entretien d'immeubles collectifs.`,
    (name, deptCount, cityCount) =>
      `En ${name}, l'économie métropolitaine concentre une population active importante sur ${deptCount} départements. Les ${cityCount} villes couvertes accueillent un parc de logements sous tension : la rénovation est un enjeu majeur.`,
    (name, deptCount, cityCount) =>
      `Première région économique, ${name} conjugue densité urbaine et dynamisme immobilier. Avec ${deptCount} départements et ${cityCount} villes, la compétition entre artisans est un gage de qualité et de réactivité.`,
    (name, deptCount, cityCount) =>
      `Le marché immobilier tendu de ${name} pousse les propriétaires à rénover plutôt qu'acheter. Sur ${deptCount} départements et ${cityCount} villes, les artisans répondent à une demande croissante en rénovation d'intérieur et mise aux normes.`,
    (name, deptCount, cityCount) =>
      `Avec ${deptCount} départements et ${cityCount} villes, ${name} concentre le plus grand bassin de professionnels du bâtiment en France. La densité de population et le parc tertiaire créent un marché artisanal où l'expertise et la disponibilité font la différence.`,
  ],
  'industrie-reconversion': [
    (name, deptCount, cityCount) =>
      `La région ${name} vit une reconversion de son tissu industriel. Ses ${deptCount} départements et ${cityCount} villes concentrent un parc immobilier de la période industrielle nécessitant une rénovation profonde.`,
    (name, deptCount, cityCount) =>
      `En ${name}, l'héritage industriel a façonné l'habitat : logements ouvriers, grands ensembles. Les ${deptCount} départements (${cityCount} villes) offrent un potentiel de rénovation considérable.`,
    (name, deptCount, cityCount) =>
      `Le dynamisme de reconversion de ${name} se traduit par une forte demande en rénovation sur ${deptCount} départements et ${cityCount} villes.`,
    (name, deptCount, cityCount) =>
      `Les programmes de réhabilitation urbaine en ${name} transforment les friches industrielles en logements et espaces de vie. Les artisans des ${deptCount} départements (${cityCount} villes) maîtrisent la reconversion de bâtiments anciens.`,
    (name, deptCount, cityCount) =>
      `La transition écologique de ${name} stimule la rénovation énergétique des logements ouvriers et HLM sur ${deptCount} départements. Les ${cityCount} villes couvertes bénéficient d'aides spécifiques aux territoires en reconversion.`,
  ],
  'agriculture-viticulture': [
    (name, deptCount, cityCount) =>
      `La région ${name}, terre de terroir, présente un patrimoine bâti rural riche sur ses ${deptCount} départements. Les ${cityCount} villes référencent des artisans spécialisés en habitat traditionnel : longères, corps de ferme, maisons de vignerons.`,
    (name, deptCount, cityCount) =>
      `En ${name}, l'économie agricole a façonné un paysage architectural unique. Les ${deptCount} départements abritent un patrimoine que les artisans des ${cityCount} villes savent restaurer.`,
    (name, deptCount, cityCount) =>
      `Le caractère agricole de ${name} se reflète dans son habitat : constructions en matériaux locaux, villages de caractère. Les artisans des ${deptCount} départements (${cityCount} villes) maîtrisent ce patrimoine rural.`,
    (name, deptCount, cityCount) =>
      `L'attractivité résidentielle de ${name} attire des néo-ruraux qui rénovent granges, moulins et dépendances agricoles. Les artisans des ${deptCount} départements (${cityCount} villes) allient savoir-faire traditionnel et techniques modernes.`,
    (name, deptCount, cityCount) =>
      `Le patrimoine viticole et agricole de ${name} génère des chantiers spécifiques : rénovation de chais, isolation de longères, restauration de toitures en tuiles plates. ${deptCount} départements et ${cityCount} villes concentrent des artisans experts.`,
  ],
  'tourisme-patrimoine': [
    (name, deptCount, cityCount) =>
      `La région ${name}, destination touristique majeure, conjugue patrimoine exceptionnel et exigences d'accueil. Les ${deptCount} départements et ${cityCount} villes abritent des artisans rompus à la restauration patrimoniale.`,
    (name, deptCount, cityCount) =>
      `En ${name}, tourisme et patrimoine appellent un niveau d'exigence élevé. Les ${deptCount} départements (${cityCount} villes) concentrent des artisans habitués aux contraintes patrimoniales.`,
    (name, deptCount, cityCount) =>
      `L'attractivité touristique de ${name} soutient une forte demande en artisanat de qualité sur ses ${deptCount} départements et ${cityCount} villes.`,
    (name, deptCount, cityCount) =>
      `Les résidences secondaires en ${name} représentent un marché important pour les artisans locaux. Sur ${deptCount} départements et ${cityCount} villes, la rénovation haut de gamme et l'entretien saisonnier alimentent l'activité tout au long de l'année.`,
    (name, deptCount, cityCount) =>
      `Le classement au patrimoine de nombreux sites en ${name} impose des contraintes spécifiques : matériaux d'origine, techniques traditionnelles, validation par les ABF. Les artisans des ${deptCount} départements (${cityCount} villes) maîtrisent ces exigences.`,
  ],
  'economie-diversifiee': [
    (name, deptCount, cityCount) =>
      `La région ${name} bénéficie d'une économie diversifiée soutenant un marché artisanal dynamique. Ses ${deptCount} départements et ${cityCount} villes accueillent un parc immobilier varié.`,
    (name, deptCount, cityCount) =>
      `En ${name}, la diversité économique se traduit par un marché de rénovation actif. Les ${deptCount} départements mêlent zones industrielles, pôles technologiques et espaces agricoles. ${cityCount} villes offrent un large choix de professionnels.`,
    (name, deptCount, cityCount) =>
      `Le tissu économique diversifié de ${name} nourrit une demande artisanale soutenue sur ses ${deptCount} départements (${cityCount} villes).`,
    (name, deptCount, cityCount) =>
      `La croissance démographique de ${name} entraîne des besoins en construction neuve et rénovation. Les ${deptCount} départements et ${cityCount} villes offrent un vivier d'artisans couvrant tous les corps de métier du bâtiment.`,
    (name, deptCount, cityCount) =>
      `L'équilibre entre urbain et rural en ${name} diversifie les besoins artisanaux : rénovation d'appartements en centre-ville, construction de maisons individuelles en périphérie. ${deptCount} départements et ${cityCount} villes sont couverts.`,
  ],
}
...
const REGION_TIPS: Record<ClimateZone, ((name: string) => string)[]> = {
  oceanique: [
    (name) =>
      `En ${name}, planifiez les travaux extérieurs entre avril et octobre. L'humidité océanique exige des matériaux résistants : menuiseries aluminium ou PVC, peintures microporeuses, enduits hydrofuges. Les artisans régionaux connaissent les produits adaptés.`,
    (name) =>
      `Pour protéger votre habitation en ${name}, investissez dans la ventilation et les traitements anti-humidité. Faites inspecter votre toiture après chaque hiver. Un démoussage régulier et un contrôle des gouttières avant les pluies sont recommandés.`,
    (name) =>
      `Les murs en pierre des maisons anciennes en ${name} nécessitent des enduits à la chaux, respirants, plutôt que du ciment qui piège l'humidité. Un artisan spécialisé en rénovation patrimoniale saura choisir le bon mortier.`,
    (name) =>
      `Le bardage bois est très répandu en ${name} mais exige un entretien régulier : lasure tous les 3-5 ans ou choix de bois autoclavé classe 3. Le bardage composite ou fibrociment offre une alternative sans entretien.`,
    (name) =>
      `En ${name}, l'installation d'une pompe à chaleur air-eau est particulièrement rentable : les hivers doux garantissent un COP élevé toute l'année. Les économies atteignent 60-70% par rapport au chauffage électrique.`,
  ],
  continental: [
    (name) =>
      `Les hivers rigoureux de ${name} nécessitent une isolation performante (R ≥ 6 en combles). Faites vérifier votre chauffage et protégez les canalisations extérieures. Planifiez les chantiers extérieurs entre mars et novembre.`,
    (name) =>
      `En ${name}, les écarts de température sollicitent les matériaux. Privilégiez les revêtements extérieurs souples et le double vitrage à isolation renforcée. Les artisans maîtrisent ces techniques d'adaptation au climat continental.`,
    (name) =>
      `Le chauffage représente le premier poste de dépense en ${name}. Avant de changer de chaudière, faites réaliser un audit énergétique : l'isolation des combles (15-25 €/m²) offre le meilleur retour sur investissement.`,
    (name) =>
      `Les caves et sous-sols en ${name} sont exposés au gel et à l'humidité. Un cuvelage étanche associé à une ventilation mécanique protège vos fondations et valorise cet espace habitable de 800 à 1 200 €/m².`,
    (name) =>
      `En ${name}, programmez l'entretien de votre chaudière entre septembre et novembre pour être prêt avant les grands froids. Le ramonage est obligatoire une à deux fois par an selon la réglementation locale.`,
  ],
  mediterraneen: [
    (name) =>
      `Le soleil de ${name} est un atout mais aussi une contrainte. Protégez vos intérieurs avec des volets et stores extérieurs. Pour les extérieurs, choisissez des enduits minéraux et peintures résistantes aux UV.`,
    (name) =>
      `En ${name}, préparez la saison des pluies : vérifiez gouttières, étanchéité et drainage dès septembre. Une isolation par l'extérieur réduit de 5 à 8°C la température intérieure en été.`,
    (name) =>
      `La pergola bioclimatique est l'aménagement extérieur le plus demandé en ${name} : lames orientables pour moduler l'ombre, intégration de brumisateurs et éclairage LED. Comptez 8 000 à 15 000 € posée par un artisan qualifié.`,
    (name) =>
  ...
  montagnard: [
    (name) =>
      `En altitude en ${name}, l'isolation doit être exceptionnelle (R > 7 en combles). Le bois nécessite un traitement régulier. Planifiez les travaux entre mai et octobre — les artisans optimisent la fenêtre d'intervention estivale.`,
    (name) =>
      `Les constructions en ${name} doivent supporter neige et gel. Choisissez des charpentes dimensionnées pour la charge de neige locale. Le chauffage au bois ou pompe à chaleur est performant avec les dernières générations d'équipements.`,
    (name) =>
      `En ${name}, le poêle à granulés est le complément idéal du chauffage central : rendement de 90%, programmable et autonome 24-48h. L'approvisionnement en granulés est facilité par la proximité des forêts.`,
    (name) =>
      `La toiture en lauze ou ardoise, traditionnelle en ${name}, requiert des artisans couvreurs spécialisés. Le coût de rénovation est élevé (150-250 €/m²) mais la durée de vie dépasse 80 ans avec un entretien minimal.`,
    (name) =>
      `Les accès aux chantiers en ${name} peuvent être difficiles en hiver. Prévoyez les livraisons de matériaux lourds avant les premières neiges et réservez les artisans dès le printemps — les carnets se remplissent vite en zone montagne.`,
  ],
  'semi-oceanique': [
    (name) =>
      `Le climat tempéré de ${name} permet des travaux quasiment toute l'année, avec une pause en décembre-janvier pour l'extérieur. L'humidité modérée justifie une bonne ventilation. Profitez du printemps et de l'automne pour les ravalements.`,
    (name) =>
      `En ${name}, l'équilibre entre isolation et ventilation est crucial. Installez une VMC hygroréglable et aérez quotidiennement. Préférez une approche globale plutôt que des interventions ponctuelles.`,
    (name) =>
      `Le ravalement de façade en ${name} est recommandé tous les 10-15 ans. Profitez-en pour ajouter une isolation par l'extérieur : le surcoût de 30-40% est couvert en partie par les aides et s'amortit en 8-12 ans.`,
    (name) =>
  ...
  tropical: [
    (name) =>
      `En ${name}, la construction doit résister aux cyclones et à l'humidité. Privilégiez les structures béton armé, les menuiseries aluminium anticorrosion et les peintures fongicides. Climatisation à entretenir trimestriellement.`,
    (name) =>
      `Le climat tropical de ${name} demande un entretien fréquent. Traitez les bois contre les termites, vérifiez l'étanchéité avant les pluies, et protégez les métaux contre la corrosion saline.`,
    (name) =>
      `L'isolation thermique en ${name} vise le confort d'été : toiture ventilée avec sous-face réfléchissante, murs en parpaings isolés par l'extérieur. La casquette solaire au-dessus des fenêtres réduit la chaleur intérieure de 3 à 5°C.`,
    (name) =>
      `Les chauffe-eau solaires sont particulièrement rentables en ${name} avec un ensoleillement quasi permanent. L'installation couvre 80-100% des besoins en eau chaude et se rentabilise en 4-6 ans.`,
    (name) =>
      `En ${name}, faites vérifier votre installation électrique tous les 5 ans : l'humidité et la salinité accélèrent la corrosion des connexions. Un tableau électrique aux normes NF C 15-100 avec protection différentielle 30 mA est impératif.`,
  ],
}

const REGION_FAQ_POOL: {
  ...
  {
    q: (name) => `Comment trouver un artisan en ${name} ?`,
    a: (name, deptCount, cityCount) =>
      `Parcourez les ${deptCount} départements de ${name} ou sélectionnez parmi les ${cityCount} villes. Choisissez le service et accédez aux artisans identifiés via les données SIREN. Devis gratuits.`,
  },
  {
    q: (name) => `Combien coûte un artisan en ${name} ?`,
    a: (name) =>
      `Les tarifs en ${name} varient selon le métier (45-90 €/h), la complexité et la zone. Les zones urbaines sont +10-25% vs rurales. Demandez plusieurs devis.`,
  },
  {
    q: (name) => `Quels travaux prioritaires en ${name} ?`,
    a: (name, _deptCount, _cityCount, climate, facts) =>
      `Le ${climate.toLowerCase()} de ${name} détermine les priorités : ${facts.slice(0, 2).join(' ; ').toLowerCase()}. L'isolation et la rénovation énergétique sont les investissements les plus rentables.`,
  },
  {
    q: (name) => `Les devis sont-ils gratuits en ${name} ?`,
    a: (name) =>
      `Oui, tous les devis via ServicesArtisans pour ${name} sont 100% gratuits et sans engagement. Jusqu'à 3 propositions de professionnels qualifiés.`,
  },
  {
    q: (name) => `Quelles aides à la rénovation en ${name} ?`,
    a: (name) =>
      `En ${name} : MaPrimeRénov' Parcours accompagné (montant variable selon revenus et travaux), éco-PTZ (jusqu'à 50 000 €), CEE, TVA réduite à 5,5%, et aides régionales. Artisan RGE requis pour les aides énergétiques.`,
  },
  {
    q: (name) => `Comment le climat impacte le bâti en ${name} ?`,
    a: (name, _deptCount, _cityCount, climate, facts) =>
      `Le ${climate.toLowerCase()} de ${name} a un impact direct : ${facts.slice(0, 3).join(' ; ').toLowerCase()}. Les artisans adaptent techniques et matériaux.`,
  },
  {
    q: (name) => `Urgence artisan en ${name} ?`,
    a: (name) =>
      `Certains artisans en ${name} proposent des interventions d'urgence, y compris le soir et le week-end : fuite, panne électrique, serrure, chauffage. Délais variables selon disponibilité et localisation.`,
  },
  {
    q: (name) => `D'où proviennent les données artisans en ${name} ?`,
    a: (name) =>
      `Les artisans en ${name} proviennent des données SIREN officielles. Chaque professionnel dispose d'un SIREN actif et d'une activité déclarée dans le bâtiment.`,
  },
  {
    q: (name) => `Meilleure saison pour rénover en ${name} ?`,
    a: (name, _deptCount, _cityCount, climate) =>
      `En ${name}, le ${climate.toLowerCase()} influence le calendrier. Extérieurs optimaux au printemps/été. Intérieurs toute l'année. Planifiez 2-3 mois à l'avance.`,
  },
  {
    q: (name) => `Comment vérifier un artisan en ${name} ?`,
    a: (name) =>
      `Contrôlez le SIRET sur l'INSEE, demandez assurance décennale et RC pro, vérifiez certifications (RGE, Qualibat), et consultez les avis pour les artisans de ${name}.`,
  },
  {
    q: (name) => `Certifications importantes en ${name} ?`,
    a: (name) =>
      `En ${name} : RGE (indispensable pour les aides), Qualibat (qualification), QualiPV/QualiSol (solaire), QualiBois/QualiPAC (chauffage). Labels garantissant un niveau de compétence vérifié.`,
  },
  {
    q: (name) => `Combien de départements en ${name} ?`,
    a: (name, deptCount, cityCount) =>
      `${name} regroupe ${deptCount} départements et ${cityCount} villes référencées. Chaque département dispose d'une page dédiée avec artisans par spécialité.`,
  },
  {
    q: (name) => `Permis de construire pour rénover en ${name} ?`,
    a: (name) =>
      `En ${name}, les travaux intérieurs sans modification de structure ne nécessitent aucune autorisation. Modifications de façade ou extensions > 5 m² : déclaration préalable. Secteurs protégés : aval ABF.`,
  },
  {
    q: (name) => `Comment comparer les devis en ${name} ?`,
    a: (name) =>
      `Pour comparer en ${name} : même périmètre, vérifiez détail des prestations, marques, délais, prix HT/TTC, conditions de paiement, assurances. Le moins cher n'est pas toujours le meilleur.`,
  },
  {
    q: (name) => `Artisans les plus demandés en ${name} ?`,
    a: (name, _deptCount, _cityCount, climate) =>
      `En ${name}, avec le ${climate.toLowerCase()}, plombiers, électriciens et chauffagistes dominent les urgences. Pour les projets planifiés : peintres, menuisiers et maçons.`,
  },
]
...
function getRegionProfile(region: import('@/lib/data/france').Region): RegionProfile {
  ...
  const economy =
    REGIONAL_ECONOMIES_LIST.find((e) => e.key === ecoKey) || REGIONAL_ECONOMIES_LIST[4]
  ...
  const mixedFacts = [
    ...facts.slice(0, 3),
    climateIssues[seed % climateIssues.length],
    climateIssues[(seed + 1) % climateIssues.length],
  ]
...
export function generateRegionContent(
  region: import('@/lib/data/france').Region,
  cityCountOverride?: number
): RegionContent {
  const seed = Math.abs(hashCode(`region-${region.slug}`))
  const profile = getRegionProfile(region)
  const deptCount = region.departments.length
  const cityCount =
    cityCountOverride ?? region.departments.reduce((acc, d) => acc + d.cities.length, 0)
  ...
  const contexteRegional = ctxTemplates[
    Math.abs(hashCode(`ctx-region-${region.slug}`)) % ctxTemplates.length
  ](region.name, deptCount, cityCount)
  ...
  const conseilsRegion = tipTemplates[
    Math.abs(hashCode(`tips-region-${region.slug}`)) % tipTemplates.length
  ](region.name)
  ...
  const faqItems = faqIndices.map((idx) => {
    ...
    return {
      question: f.q(region.name),
      answer: f.a(region.name, deptCount, cityCount, profile.climateLabel, profile.keyFacts),
    }
  })
...
const VILLE_INTROS: ((p: VilleIntroParams) => string)[] = [
  ...
  (p) =>
    `${p.name} (${p.deptCode}), ${p.pop} habitants, bénéficie d'artisans qualifiés intervenant sur l'ensemble de la commune et ses quartiers. En ${p.region}, le climat ${p.climate.toLowerCase()} impose des exigences spécifiques que nos professionnels maîtrisent parfaitement.`,
  (p) =>
    `Dans le ${p.dept} (${p.region}), ${p.name} et ses ${p.pop} habitants profitent d'un réseau d'artisans de confiance. Le contexte ${p.climate.toLowerCase()} local oriente les priorités en matière d'entretien et de rénovation des logements.`,
  (p) =>
    `${p.name}, ${p.sizeLabel.toLowerCase()} de ${p.pop} habitants ${getDeptPreposition(p.dept)} (${p.deptCode}), dispose d'un parc immobilier aux besoins spécifiques liés au climat ${p.climate.toLowerCase()} ${getRegionArticle(p.region)}.`,
  (p) =>
  ...
  (p) =>
    `Implantée ${getRegionPreposition(p.region)}, ${p.name} (${p.deptCode}) réunit ${p.pop} habitants. Le climat ${p.climate.toLowerCase()} et les caractéristiques du bâti local définissent les besoins prioritaires en artisanat du bâtiment.`,
]

interface VilleCtxParams {
  name: string
  pop: string
  qc: number
}
...
const VILLE_CONTEXTS: Record<CitySize, ((p: VilleCtxParams) => string)[]> = {
  metropole: [
    (p) =>
      `Avec ${p.pop} habitants répartis sur ${p.qc > 0 ? p.qc + ' quartiers' : 'de nombreux quartiers'}, ${p.name} présente une forte demande en rénovation d'immeubles anciens, mise aux normes électriques et plomberie de copropriété. Les délais d'intervention sont courts grâce à la densité de professionnels disponibles.`,
    (p) =>
      `Métropole de ${p.pop} habitants, ${p.name} concentre des enjeux majeurs : rénovation énergétique des logements collectifs, entretien des installations communes et adaptation aux nouvelles réglementations thermiques (RE2020).`,
    (p) =>
      `À ${p.name}, la densité urbaine et les ${p.pop} habitants génèrent une demande soutenue en services d'artisanat. Les immeubles anciens nécessitent des interventions régulières, tandis que les constructions neuves requièrent des finitions de qualité.`,
    (p) =>
      `Le marché du bâtiment à ${p.name} (${p.pop} hab.) est structuré autour des copropriétés : réfection des parties communes, mise en conformité des colonnes montantes, ravalement de façade et rénovation des ascenseurs mobilisent régulièrement les artisans.`,
    (p) =>
      `${p.name} connaît une dynamique de rénovation soutenue : DPE obligatoire, interdiction progressive des passoires thermiques et exigences de copropriété poussent les ${p.pop} habitants à solliciter des artisans qualifiés pour améliorer la performance énergétique de leur logement.`,
  ],
  'grande-ville': [
    (p) =>
      `Grande ville de ${p.pop} habitants${p.qc > 0 ? ` avec ${p.qc} quartiers identifiés` : ''}, ${p.name} combine des besoins variés : entretien du parc ancien en centre-ville et travaux neufs dans les zones d'extension urbaine.`,
    (p) =>
      `${p.name} et ses ${p.pop} habitants constituent un bassin d'activité artisanale dynamique. La diversité du bâti — du centre historique aux lotissements périphériques — appelle des compétences variées.`,
    (p) =>
      `Avec ${p.pop} habitants, ${p.name} offre un marché artisanal équilibré entre rénovation et construction neuve. Les professionnels locaux interviennent aussi bien en centre-ville que dans les quartiers résidentiels.`,
    (p) =>
      `À ${p.name}${p.qc > 0 ? ` et ses ${p.qc} quartiers` : ''}, la transformation urbaine génère des besoins multiples : réhabilitation de friches, aménagement de logements et entretien d'un parc immobilier vieillissant pour les ${p.pop} habitants.`,
    (p) =>
      `Les ${p.pop} habitants de ${p.name} bénéficient d'un tissu artisanal dense et spécialisé. Entre rénovation du centre historique et aménagement des zones résidentielles périphériques, les professionnels du bâtiment sont très sollicités.`,
  ],
  'ville-moyenne': [
    (p) =>
      `Ville moyenne de ${p.pop} habitants, ${p.name} se caractérise par un habitat principalement pavillonnaire. Les besoins portent sur l'entretien courant, la rénovation énergétique et l'amélioration du confort des maisons individuelles.`,
    (p) =>
      `À ${p.name} (${p.pop} hab.), le parc immobilier mêle constructions traditionnelles et pavillons plus récents. Les artisans locaux sont sollicités pour des travaux d'isolation, de chauffage et de mise aux normes.`,
    (p) =>
      `Commune de ${p.pop} habitants, ${p.name} présente un tissu résidentiel à taille humaine. Les propriétaires y privilégient les artisans de proximité pour leurs travaux de rénovation et d'entretien.`,
    (p) =>
      `${p.name} (${p.pop} hab.) offre un cadre de vie résidentiel où les maisons individuelles dominent. Les demandes portent sur l'extension de logements, la rénovation de cuisines et salles de bain, et l'amélioration de l'isolation thermique.`,
    (p) =>
      `Avec ${p.pop} habitants, ${p.name} connaît un renouvellement progressif de son parc immobilier. Les propriétaires investissent dans la modernisation du chauffage, l'isolation par l'extérieur et la réfection des toitures vieillissantes.`,
  ],
  'petite-ville': [
    (p) =>
      `Petite commune de ${p.pop} habitants, ${p.name} possède un patrimoine bâti souvent ancien nécessitant des compétences spécifiques en rénovation. Les artisans intervenant sur la commune connaissent les matériaux traditionnels locaux.`,
    (p) =>
      `À ${p.name} (${p.pop} hab.), l'habitat traditionnel prédomine. Les travaux portent principalement sur la rénovation, l'isolation et la mise aux normes des installations existantes.`,
    (p) =>
      `Commune de ${p.pop} habitants, ${p.name} bénéficie d'artisans polyvalents capables d'intervenir sur des bâtis anciens comme sur des constructions plus récentes. La proximité est un atout majeur pour la réactivité.`,
    (p) =>
      `${p.name} et ses ${p.pop} habitants comptent sur des artisans de proximité pour entretenir un patrimoine bâti où le charme de l'ancien côtoie les exigences de confort moderne : isolation, chauffage performant et mise aux normes électriques.`,
    (p) =>
      `Dans cette commune de ${p.pop} habitants, les chantiers de rénovation sont variés : restauration de façades en matériaux locaux, remplacement de menuiseries, réfection de toitures et création d'extensions pour adapter les logements aux besoins actuels.`,
  ],
}
...
const VILLE_TIPS: Record<ClimateZone, ((name: string) => string)[]> = {
  oceanique: [
    (name) =>
      `À ${name}, le climat océanique entraîne une humidité importante. Faites vérifier l'étanchéité de votre toiture chaque automne et traitez les murs contre les remontées capillaires. Les façades doivent être nettoyées et imperméabilisées tous les 10 ans.`,
    (name) =>
      `En climat océanique à ${name}, privilégiez les matériaux résistants à l'humidité pour vos menuiseries extérieures. Le PVC ou l'aluminium offrent une meilleure longévité que le bois non traité dans ces conditions.`,
    (name) =>
      `L'air marin à ${name} accélère la corrosion des métaux. Optez pour des gouttières en zinc ou en PVC plutôt qu'en acier, et faites traiter les garde-corps et ferronneries avec une peinture antirouille marine tous les 5 ans.`,
    (name) =>
      `À ${name}, la VMC est essentielle pour évacuer l'excès d'humidité intérieure. Faites nettoyer les bouches d'extraction chaque année et vérifiez le bon fonctionnement du caisson moteur pour prévenir les moisissures.`,
    (name) =>
      `Le climat de ${name} favorise la pousse de mousses sur les toitures et terrasses. Un nettoyage basse pression suivi d'un traitement anti-mousse au printemps prolonge la durée de vie de vos revêtements extérieurs.`,
  ],
  continental: [
    (name) =>
      `À ${name}, les écarts de température entre été et hiver sollicitent fortement les façades et les joints. Prévoyez un contrôle de chaudière avant l'hiver et vérifiez l'isolation des combles pour réduire votre facture énergétique.`,
    (name) =>
      `Le climat continental de ${name} commande une vigilance particulière sur les canalisations en période de gel. Pensez à purger les circuits extérieurs et à calorifuger les tuyaux exposés avant les premières gelées.`,
    (name) =>
      `À ${name}, les dallages extérieurs souffrent des cycles gel-dégel. Privilégiez des matériaux résistants au gel (grès cérame, pierre naturelle) et prévoyez des joints souples pour absorber les dilatations thermiques.`,
    (name) =>
      `En hiver à ${name}, la consommation de chauffage peut représenter jusqu'à 70% de la facture énergétique. L'isolation des combles perdus est le premier investissement à réaliser pour un retour rapide.`,
    (name) =>
      `Les étés chauds et les hivers froids à ${name} sollicitent les volets roulants et les fermetures. Faites réviser les mécanismes chaque automne et graissez les rails pour éviter les blocages en plein hiver.`,
  ],
  mediterraneen: [
    (name) =>
      `À ${name}, la chaleur estivale rend la climatisation indispensable. Faites entretenir votre système chaque printemps et vérifiez que l'isolation de vos combles limite les surchauffes. Les volets roulants sont un investissement rentable.`,
    (name) =>
      `En climat méditerranéen à ${name}, les épisodes cévenols peuvent causer des infiltrations. Vérifiez régulièrement l'état de vos gouttières et de l'étanchéité de votre toiture, surtout en automne.`,
    (name) =>
      `À ${name}, le soleil intense dégrade rapidement les peintures extérieures. Optez pour des peintures acryliques haute résistance UV et prévoyez un rafraîchissement des façades tous les 8 à 10 ans.`,
    (name) =>
      `La sécheresse à ${name} provoque un retrait des argiles qui peut fissurer les fondations. Surveillez l'apparition de fissures sur vos murs et consultez un maçon si elles évoluent, surtout après un été sec.`,
    (name) =>
      `En été à ${name}, les stores et brise-soleil sont sollicités quotidiennement. Faites réviser les mécanismes de vos volets chaque printemps et vérifiez la tension des toiles de store pour garantir leur longévité.`,
  ],
  montagnard: [
    (name) =>
      `À ${name}, le poids de la neige sur les toitures nécessite une charpente renforcée et un déneigement régulier. Faites inspecter votre couverture au printemps pour repérer les dégâts causés par le gel et la neige.`,
    (name) =>
      `En zone de montagne à ${name}, l'isolation thermique est primordiale. Les doubles ou triples vitrages et l'isolation des combles par l'extérieur permettent de réduire significativement les dépenses de chauffage.`,
    (name) =>
      `À ${name}, les gouttières et descentes d'eau sont mises à rude épreuve par le gel. Installez des câbles chauffants dans les chéneaux pour éviter la formation de barrages de glace qui endommagent les toitures.`,
    (name) =>
      `Le bois de charpente à ${name} subit d'importants écarts d'humidité entre saisons. Un traitement fongicide et insecticide tous les 10 ans protège la structure contre les champignons et les insectes xylophages.`,
    (name) =>
      `En altitude à ${name}, les fondations sont exposées aux mouvements de terrain liés au dégel. Assurez un drainage efficace autour de votre maison et vérifiez l'état des regards et caniveaux chaque printemps.`,
  ],
  'semi-oceanique': [
    (name) =>
      `À ${name}, le climat semi-océanique combine humidité modérée et hivers frais. Un entretien annuel de la chaudière et une vérification de la VMC garantissent un confort optimal toute l'année.`,
    (name) =>
      `En climat semi-océanique à ${name}, les façades sont exposées aux pluies fréquentes mais modérées. Un ravalement tous les 15 ans et un traitement hydrofuge préventif protègent durablement votre bien.`,
    (name) =>
      `À ${name}, l'humidité modérée mais constante favorise la condensation dans les pièces mal ventilées. Vérifiez que votre VMC fonctionne correctement et aérez quotidiennement, même en hiver, pour prévenir les moisissures.`,
    (name) =>
      `Le climat de ${name} sollicite modérément mais régulièrement les toitures. Un contrôle visuel annuel des tuiles et de la zinguerie permet de détecter les petites réparations avant qu'elles ne deviennent coûteuses.`,
    (name) =>
      `À ${name}, les peintures extérieures tiennent généralement 10 à 12 ans grâce au climat tempéré. Profitez du printemps pour inspecter les boiseries, volets et portails et planifier les retouches nécessaires.`,
  ],
  tropical: [
    (name) =>
      `À ${name}, le climat tropical nécessite des matériaux résistants à l'humidité et aux termites. Privilégiez les traitements préventifs du bois et une ventilation naturelle efficace pour limiter la climatisation.`,
    (name) =>
      `En climat tropical à ${name}, les cyclones nécessitent des menuiseries renforcées et une toiture solidement ancrée. Faites vérifier les fixations de votre charpente avant chaque saison cyclonique.`,
    (name) =>
      `À ${name}, la climatisation fonctionne souvent en continu. Un entretien trimestriel des filtres et un contrôle annuel du circuit frigorifique par un professionnel certifié prolongent la durée de vie de l'appareil.`,
    (name) =>
      `L'air salin à ${name} corrode rapidement les installations électriques extérieures. Faites vérifier les coffrets et les protections différentielles chaque année par un électricien pour garantir votre sécurité.`,
    (name) =>
      `En zone tropicale à ${name}, les peintures et enduits extérieurs se dégradent en 3 à 5 ans. Choisissez des produits spécifiques tropicaux et planifiez un rafraîchissement régulier pour protéger votre maçonnerie.`,
  ],
}

interface VilleFaqParams {
  name: string
  pop: string
  dept: string
  deptCode: string
...
const VILLE_FAQ_POOL: { q: (name: string) => string; a: (p: VilleFaqParams) => string }[] = [
  ...
  },
  {
    ...
    a: (p) =>
      `À ${p.name} (${p.dept}), nos artisans interviennent sur les principaux corps de métier du bâtiment : plomberie, électricité, serrurerie, chauffage, climatisation, couverture, maçonnerie, peinture, menuiserie, carrelage, façade, vitrerie, terrassement, aménagement paysager et domotique.`,
  },
  {
    q: (name) => `Comment obtenir un devis gratuit à ${name} ?`,
    a: (p) =>
      `Cliquez sur "Obtenir mon devis gratuit", décrivez votre projet en quelques clics, et recevez un devis personnalisé d'artisans qualifiés intervenant à ${p.name}. Le service est 100% gratuit et sans engagement.`,
  },
  {
  ...
  },
  {
    ...
    a: (p) =>
      `Comparez les devis reçus en tenant compte du prix, des délais, des garanties proposées et de l'expérience. À ${p.name}, privilégiez les artisans ayant une bonne connaissance du bâti local et du climat ${p.climate.toLowerCase()}.`,
  },
  {
    q: (name) => `Faut-il un permis de construire pour des travaux à ${name} ?`,
    a: (p) =>
      `Cela dépend de la nature des travaux. À ${p.name}, les travaux modifiant l'aspect extérieur (façade, toiture, extension) nécessitent généralement une déclaration préalable en mairie. Pour les extensions de plus de 20 m², un permis de construire est requis.`,
  },
  {
    q: (name) => `Quelles aides financières pour les travaux à ${name} ?`,
    a: (p) =>
      `Les habitants de ${p.name} (${p.dept}) peuvent bénéficier de MaPrimeRénov', de l'éco-PTZ, des CEE et des aides de l'ANAH pour la rénovation énergétique. Certaines collectivités locales proposent des aides complémentaires.`,
  },
  {
    q: (name) => `Quel artisan pour une rénovation énergétique à ${name} ?`,
    a: (p) =>
      `Pour une rénovation énergétique à ${p.name}, faites appel à un artisan RGE (Reconnu Garant de l'Environnement). Cela conditionne l'accès aux aides publiques. Les travaux courants incluent l'isolation, le chauffage et la ventilation.`,
  },
  {
    q: (name) => `Les artisans à ${name} sont-ils assurés ?`,
    a: (p) =>
      `Les artisans du bâtiment à ${p.name} sont tenus de souscrire une assurance décennale couvrant les dommages pendant 10 ans. Demandez systématiquement une attestation d'assurance avant le début des travaux.`,
  },
  {
    q: (name) => `Combien de temps durent les travaux courants à ${name} ?`,
    a: (p) =>
      `La durée varie selon le type d'intervention : un dépannage plomberie ou serrurerie prend généralement 1 à 2 heures, une rénovation de salle de bain 1 à 2 semaines, et une rénovation complète de maison à ${p.name} peut s'étendre sur plusieurs mois.`,
  },
  {
    q: (name) => `Peut-on faire intervenir un artisan le week-end à ${name} ?`,
    a: (p) =>
      `Certains artisans à ${p.name} proposent des interventions le week-end, notamment pour les urgences (plombier, serrurier, électricien). Un supplément de 20 à 50% est généralement appliqué pour les interventions hors heures ouvrées.`,
  },
  {
    q: (name) => `Comment signaler un problème avec un artisan à ${name} ?`,
    a: (p) =>
      `En cas de litige avec un artisan à ${p.name}, commencez par une réclamation écrite. Si le différend persiste, contactez le médiateur de la consommation ou la DGCCRF ${getDeptArticle(p.dept)}. L'assurance décennale couvre les malfaçons structurelles.`,
  },
]
...
export function generateVilleContent(ville: import('@/lib/data/france').Ville): VilleContent {
  ...
  const intro = introFn({
    ...
    sizeLabel: profile.citySizeLabel,
  })

  const ctxTemplates = VILLE_CONTEXTS[profile.citySize]
  const contexteUrbain = ctxTemplates[
  ...
  const conseilsVille = tipTemplates[
    Math.abs(hashCode(`tips-ville-${ville.slug}`)) % tipTemplates.length
  ](ville.name)
