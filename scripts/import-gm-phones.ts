 ...
 *
 * 2 Iterations (safe thresholds to avoid mismatches):
 *   1. Strict name match within same department + trade (threshold 0.50)
 *   2. Relaxed: dept+city, dept+trade partial, first-word match (threshold 0.40)
 *
 * ⚠️  L'itération 3 "aggressive" (seuil 0.10) a été SUPPRIMÉE car elle assignait
 *     des numéros au mauvais artisan.
 *
...
async function main() {
  ...
  console.log('\n' + '═'.repeat(70))
  console.log('  ITERATION 1: Department + Trade + Fuzzy Name (threshold 0.50)')
  console.log('═'.repeat(70))
  ...
  for (const [dept, records] of byDept) {
    ...
    for (const rec of records) {
      ...
      if (bestProvider && bestScore >= 0.50) {
        try {
  ...
  console.log('\n' + '═'.repeat(70))
  console.log('  ITERATION 2: Relaxed — First-word match, cross-trade in dept (threshold 0.40)')
  console.log('═'.repeat(70))
  ...
  for (const [dept, records] of byDept2) {
    ...
    for (const rec of records) {
      ...
      // Strategy B: Fuzzy name match across ALL specialties (threshold 0.50)
      if (!bestProvider || bestScore < 0.40) {
        for (const prov of providers) {
          if (prov.phone !== null) continue
          const score = diceSimilarity(rec.name, prov.name)
      ...
      if ((!bestProvider || bestScore < 0.40) && rec.city) {
  ...
  const totalMatched = matched1 + matched2
  ...
  console.log(`  ─────────────────────────────`)
  console.log(`  Iteration 1 (strict):        ${matched1}`)
