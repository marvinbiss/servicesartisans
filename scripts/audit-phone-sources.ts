...
async function fetchAllProvidersWithPhones(): Promise<ProviderRow[]> {
  ...
  while (true) {
    const { data, error } = await supabase
      .from('providers')
      .select(
        'id, name, phone, siret, source, source_api, created_at, updated_at, claimed_at, address_department'
      )
      .not('phone', 'is', null)
      .neq('phone', '')
...
async function main() {
  ...
  const fuzzyDepts = Object.entries(deptBreakdown.enrichment_fuzzy)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 20)
  ...
  const sameDayDepts = Object.entries(deptBreakdown.enrichment_same_day)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 20)
